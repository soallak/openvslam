...
inline char Option::short_name() const {
  if (!short_name_.empty()) return short_name_[0];
  return 0;
}
...
inline T Value<T>::value(size_t idx) const {
  if (!this->is_set() && default_) return *default_;
...
inline T Value<T>::get_default() const {
  if (!has_default()) throw std::runtime_error("no default value set");
...
inline void Value<T>::parse(OptionName what_name, const char* value) {
  T parsed_value;
  std::string strValue;
  if (value != nullptr) strValue = value;
...
inline void Value<T>::update_reference() {
  if (this->assign_to_) {
    if (this->is_set() || default_) *this->assign_to_ = value();
  }
}
...
inline Option_ptr OptionParser::find_option(
    const std::string& long_name) const {
  for (const auto& option : options_)
    if (option->long_name() == long_name) return option;
  return nullptr;
}
...
template <typename T>
inline std::shared_ptr<T> OptionParser::get_option(
    const std::string& long_name) const {
  Option_ptr option = find_option(long_name);
  if (!option) throw std::invalid_argument("option not found: " + long_name);
...
inline void OptionParser::parse(int argc, const char* const argv[]) {
  ...
  for (auto& opt : options_) opt->clear();

  for (int n = 1; n < argc; ++n) {
    const std::string arg(argv[n]);
    if (arg == "--") {
      /// from here on only non opt args
      for (int m = n + 1; m < argc; ++m) non_option_args_.emplace_back(argv[m]);
    } else if (arg.find("--") == 0) {
      /// long option arg
      ...
      if (unknown) unknown_options_.push_back(arg);
    } else {
...
inline std::string ConsoleOptionPrinter::to_string(Option_ptr option) const {
  ...
  if (option->short_name() != 0) {
    ...
    if (!option->long_name().empty()) line << ", ";
  } else
    line << "  ";
  if (!option->long_name().empty()) line << "--" << option->long_name();

  if (option->argument_type() == Argument::required) {
    line << " arg";
    std::stringstream defaultStr;
    if (option->get_default(defaultStr)) {
      if (!defaultStr.str().empty()) line << " (=" << defaultStr.str() << ")";
    }
  } else if (option->argument_type() == Argument::optional) {
    std::stringstream defaultStr;
    if (option->get_default(defaultStr))
...
inline std::string ConsoleOptionPrinter::print(
    const Attribute& max_attribute) const {
  if (option_parser_ == nullptr) return "";
...
inline std::string GroffOptionPrinter::print(
    const Attribute& max_attribute) const {
  if (option_parser_ == nullptr) return "";
...
inline std::string BashCompletionOptionPrinter::print(
  ...
  for (const auto& option : option_parser_->options()) {
    if (option->attribute() > Attribute::hidden) {
      if (option->short_name() != 0) s << "-" << option->short_name() << " ";
      if (!option->long_name().empty()) s << "--" << option->long_name() << " ";
    }
  }
