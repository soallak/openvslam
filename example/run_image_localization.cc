...
int main(int argc, char* argv[]) {
  ...
  auto img_dir_path = op.add<popl::Value<std::string>>(
      "i", "img-dir", "directory path which contains images");
  ...
  if (cfg->camera_->setup_type_ == openvslam::camera::setup_type_t::Monocular) {
    mono_localization(cfg, vocab_file_path->value(), img_dir_path->value(),
                      mask_img_path->value(), map_db_path->value(),
                      mapping->is_set(), frame_skip->value(),
                      no_sleep->is_set(), auto_term->is_set());
  } else {
