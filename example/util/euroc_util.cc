...
euroc_sequence::euroc_sequence(const std::string& seq_dir_path) {
  ...
  while (!ifs_timestamp.eof()) {
    ...
    if (!s.empty()) {
      ...
      left_img_file_paths_.push_back(left_img_dir_path +
                                     std::to_string(timestamp) + ".png");
      right_img_file_paths_.push_back(right_img_dir_path +
                                      std::to_string(timestamp) + ".png");
    }
  }
...
std::vector<euroc_sequence::frame> euroc_sequence::get_frames() const {
  ...
  for (unsigned int i = 0; i < timestamps_.size(); ++i) {
    frames.emplace_back(frame{left_img_file_paths_.at(i),
                              right_img_file_paths_.at(i), timestamps_.at(i)});
  }
  return frames;
}
