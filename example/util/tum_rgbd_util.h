...
class tum_rgbd_sequence {
 public:
  struct frame {
    frame(const std::string& rgb_img_path, const std::string& depth_img_path,
  ...
  explicit tum_rgbd_sequence(const std::string& seq_dir_path,
                             const double min_timediff_thr = 0.1);
 ...
 private:
  ...
  std::vector<img_info> acquire_image_information(
