...
tum_rgbd_sequence::tum_rgbd_sequence(const std::string& seq_dir_path,
                                     const double min_timediff_thr) {
  // listing up the files in rgb/ and depth/ directories
  const auto rgb_img_infos =
      acquire_image_information(seq_dir_path, seq_dir_path + "/rgb.txt");
  const auto depth_img_infos =
      acquire_image_information(seq_dir_path, seq_dir_path + "/depth.txt");
  ...
  for (const auto& rgb_img_info : rgb_img_infos) {
    ...
    double min_timediff =
        std::abs(rgb_img_timestamp - nearest_depth_img_timestamp);
    ...
    timestamps_.push_back((rgb_img_timestamp + nearest_depth_img_timestamp) /
                          2.0);
...
std::vector<tum_rgbd_sequence::frame> tum_rgbd_sequence::get_frames() const {
  ...
  for (unsigned int i = 0; i < timestamps_.size(); ++i) {
    frames.emplace_back(frame{rgb_img_file_paths_.at(i),
                              depth_img_file_paths_.at(i), timestamps_.at(i)});
  }
  return frames;
}

std::vector<tum_rgbd_sequence::img_info>
tum_rgbd_sequence::acquire_image_information(
    const std::string& seq_dir_path,
  ...
  if (!ifs_timestamp) {
    throw std::runtime_error("Could not load a timestamp file from " +
                             timestamp_file_path);
  }
  ...
  while (!ifs_timestamp.eof()) {
    ...
    if (!s.empty()) {
      ...
      img_infos.emplace_back(
          img_info{timestamp, seq_dir_path + "/" + img_file_name});
    }
  }
