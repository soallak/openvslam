...
void mono_tracking(const std::shared_ptr<openvslam::config>& cfg,
                   ...
                   const unsigned int cam_num, const std::string& mask_img_path,
                   const float scale, const std::string& map_db_path) {
  // load the mask image
  const cv::Mat mask = mask_img_path.empty()
                           ? cv::Mat{}
...
void stereo_tracking(const std::shared_ptr<openvslam::config>& cfg,
                     ...
                     const std::string& mask_img_path, const float scale,
  ...
  std::thread thread([&]() {
    while (is_not_end) {
      ...
      for (int i = 0; i < 2; i++) {
        if (scale != 1.0) {
          cv::resize(frames[i], frames[i], cv::Size(), scale, scale,
                     cv::INTER_LINEAR);
        }
      }
      rectifier.rectify(frames[0], frames[1], frames_rectified[0],
                        frames_rectified[1]);

      const auto tp_1 = std::chrono::steady_clock::now();

      // input the current frame and estimate the camera pose
      SLAM.feed_stereo_frame(frames_rectified[0], frames_rectified[1],
                             timestamp, mask);
...
int main(int argc, char* argv[]) {
  ...
  auto cam_num =
      op.add<popl::Value<unsigned int>>("n", "number", "camera number");
  ...
  auto scale =
      op.add<popl::Value<float>>("s", "scale", "scaling ratio of images", 1.0);
  ...
  if (cfg->camera_->setup_type_ == openvslam::camera::setup_type_t::Monocular) {
    mono_tracking(cfg, vocab_file_path->value(), cam_num->value(),
                  mask_img_path->value(), scale->value(), map_db_path->value());
  } else if (cfg->camera_->setup_type_ ==
             openvslam::camera::setup_type_t::Stereo) {
    stereo_tracking(cfg, vocab_file_path->value(), cam_num->value(),
                    mask_img_path->value(), scale->value(),
                    map_db_path->value());
  } else {
