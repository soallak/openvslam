...
void create_keypoints(const Mat33_t& rot_cw, const Vec3_t& trans_cw,
                      const Mat33_t& cam_matrix,
                      const eigen_alloc_vector<Vec3_t>& landmarks,
...
void create_keypoints(const Mat33_t& rot_cw, const Vec3_t& trans_cw,
  ...
  for (unsigned int i = 0; i < num_landmarks; ++i) {
    ...
    Vec3_t pos_c = rot_cw * pos_w + trans_cw;
    keypts.at(i).pt.x =
        cam_matrix(0, 0) * pos_c(0) / pos_c(2) + cam_matrix(0, 2);
    keypts.at(i).pt.y =
        cam_matrix(1, 1) * pos_c(1) / pos_c(2) + cam_matrix(1, 2);
  }
