...
TEST(angle_checker, check_matches_top_1) {
  ...
  for (const auto match : valid_matches) {
    ...
    EXPECT_TRUE(match != 324 && match != 127 && match != 0 && match != 60 &&
                match != 180 && match != 270);
  }
...
TEST(angle_checker, check_matches_top_2) {
  ...
  for (const auto match : valid_matches) {
    ...
    EXPECT_TRUE(match != 127 && match != 0 && match != 60 && match != 180 &&
                match != 270);
  }
...
TEST(angle_checker, check_matches_all) {
  ...
  EXPECT_TRUE(!valid_matches.empty());
  for (const auto match : valid_matches) {
    EXPECT_TRUE(match == 35 || match == 324 || match == 127 || match == 0 ||
                match == 60 || match == 180 || match == 270);
  }
