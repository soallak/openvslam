...
cv::Mat draw_lines(const cv::Mat& img, const unsigned int num_segments = 4) {
  ...
  for (unsigned int i = 0; i < num_segments; ++i) {
    cv::line(lined_img, cv::Point2i(i * img.cols / num_segments, 0),
             cv::Point2i(i * img.cols / num_segments, img.rows - 1),
             cv::Scalar(255, 255, 255), 1);
  }
  for (unsigned int i = 0; i < num_segments; ++i) {
    cv::line(lined_img, cv::Point2i(0, i * img.rows / num_segments),
             cv::Point2i(img.cols - 1, i * img.rows / num_segments),
             cv::Scalar(255, 255, 255), 1);
  }
  return lined_img;
}
...
TEST(orb_extractor, extract_toy_sample_1) {
  ...
  img = 255;
  cv::rectangle(img, cv::Point2i(300, 300), cv::Point2i(600, 600),
...
TEST(orb_extractor, extract_toy_sample_2) {
  ...
  img = 255;
  cv::rectangle(img, cv::Point2i(0, 0), cv::Point2i(1800, 1800), cv::Scalar(0),
                -1, cv::LINE_AA);
  // mask (無効)
  const auto mask = cv::Mat();
...
TEST(orb_extractor, extract_without_mask_1) {
  ...
  // image
  const auto img =
      cv::imread(std::string(TEST_DATA_DIR) + "./equirectangular_image_001.jpg",
                 cv::IMREAD_GRAYSCALE);
  // mask (無効)
  const auto mask = cv::Mat();
...
TEST(orb_extractor, extract_without_mask_2) {
  ...
  // image
  const auto img =
      cv::imread(std::string(TEST_DATA_DIR) + "./equirectangular_image_002.jpg",
                 cv::IMREAD_GRAYSCALE);
  // mask (無効)
  const auto mask = cv::Mat();
...
TEST(orb_extractor, extract_with_image_mask_1) {
  ...
  const std::string show_keypts_str =
      (show_keypts != nullptr) ? show_keypts : "";
  if (show_keypts_str == "YES" || show_keypts_str == "ON" ||
      show_keypts_str == "TRUE" || show_keypts_str == "1") {
    cv::Mat vis;
...
TEST(orb_extractor, extract_with_image_mask_3) {
  ...
  auto mask = cv::Mat(img.rows, img.cols, CV_8UC1);
  mask = 1;
  cv::circle(mask, cv::Point2i(img.cols / 2, img.rows / 2), radius,
             cv::Scalar(0), -1, cv::LINE_AA);
  ...
  if (show_keypts_str == "YES" || show_keypts_str == "ON" ||
    ...
    cv::Mat vis;
    cv::drawKeypoints(img, keypts, vis);
    cv::circle(vis, cv::Point2i(img.cols / 2, img.rows / 2), radius,
               cv::Scalar(255, 255, 255), 1);
...
TEST(orb_extractor, extract_with_rectangle_mask_1) {
  ...
  // mask (上下20%をマスク)
  auto extractor = feature::orb_extractor(
      &params, 1000, {{0.0, 1.0, 0.0, 0.2}, {0.0, 1.0, 0.8, 1.0}});
...
TEST(orb_extractor, extract_with_rectangle_mask_2) {
  ...
  // mask (上下20%をマスク)
  auto extractor = feature::orb_extractor(
      &params, 1000, {{0.0, 0.2, 0.0, 1.0}, {0.8, 1.0, 0.0, 1.0}});
...
TEST(orb_extractor, extract_toy_sample_3) {
  ...
  img = 255;
  cv::rectangle(img, cv::Point2i(300, 600), cv::Point2i(600, 1200),
                cv::Scalar(0), -1, cv::LINE_AA);
  // mask (無効)
  const auto mask = cv::Mat();
