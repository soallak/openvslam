...
TEST(orb_params, calc_scale_factors) {
  ...
  const auto scale_factors =
      feature::orb_params::calc_scale_factors(num_scale_levels, scale_factor);
...
TEST(orb_params, calc_inv_scale_factors) {
  ...
  const auto inv_scale_factors = feature::orb_params::calc_inv_scale_factors(
      num_scale_levels, scale_factor);

  for (unsigned int level = 0; level < num_scale_levels; ++level) {
    EXPECT_FLOAT_EQ(inv_scale_factors.at(level),
                    std::pow((1.0f / scale_factor), level));
  }
}
...
TEST(orb_params, calc_level_sigma_sq) {
  ...
  const auto level_sigma_sq =
      feature::orb_params::calc_level_sigma_sq(num_scale_levels, scale_factor);
  float scale_factor_at_level = 1.0;
  for (unsigned int level = 0; level < num_scale_levels; ++level) {
    EXPECT_FLOAT_EQ(level_sigma_sq.at(level),
                    scale_factor_at_level * scale_factor_at_level);
...
TEST(orb_params, calc_inv_level_sigma_sq) {
  ...
  const auto inv_level_sigma_sq = feature::orb_params::calc_inv_level_sigma_sq(
      num_scale_levels, scale_factor);
  float scale_factor_at_level = 1.0;
  for (unsigned int level = 0; level < num_scale_levels; ++level) {
    EXPECT_FLOAT_EQ(inv_level_sigma_sq.at(level),
                    1.0f / (scale_factor_at_level * scale_factor_at_level));
    scale_factor_at_level = scale_factor * scale_factor_at_level;
  }
}
