...
TEST(fundamental_solver, linear_solve) {
  ...
  const Mat33_t rot_1 = util::converter::to_rot_mat(
      205.0 * M_PI / 180.0 * Vec3_t{4, -6, 2}.normalized());
  const Vec3_t trans_1 = Vec3_t(-28.1, -63.3, 43.4);
  const Mat33_t rot_2 = util::converter::to_rot_mat(
      -15.0 * M_PI / 180.0 * Vec3_t{5, 1, -3}.normalized());
  ...
  Mat33_t true_F_21 = solve::fundamental_solver::create_F_21(
      rot_1, trans_1, cam_matrix_1, rot_2, trans_2, cam_matrix_2);
...
TEST(fundamental_solver, ransac_solve_without_noise) {
  ...
  // create two-view poses
  const Mat33_t rot_1 = util::converter::to_rot_mat(
      -105.0 * M_PI / 180.0 * Vec3_t{1, 10, 3}.normalized());
  const Vec3_t trans_1 = Vec3_t(-49.1, -25.3, -3.4);
  const Mat33_t rot_2 = util::converter::to_rot_mat(
      275.0 * M_PI / 180.0 * Vec3_t{-5, 5, -4}.normalized());
...
TEST(fundamental_solver, ransac_solve_with_noise) {
  ...
  // create two-view poses
  const Mat33_t rot_1 = util::converter::to_rot_mat(
      54.0 * M_PI / 180.0 * Vec3_t{5, 3, -2}.normalized());
  const Vec3_t trans_1 = Vec3_t(40.3, -31.6, 58.4);
  const Mat33_t rot_2 = util::converter::to_rot_mat(
      -21.0 * M_PI / 180.0 * Vec3_t{-2, -5, 6}.normalized());
...
TEST(fundamental_solver, decompose) {
  ...
  solve::fundamental_solver::decompose(true_F_21, cam_matrix_1, cam_matrix_2,
                                       rots, transes);
...
TEST(fundamental_solver, create) {
  ...
  const Vec3_t trans_21 = trans_2 - rot_21 * trans_1;
  Mat33_t skew_21;
  skew_21 << 0, -trans_21(2), trans_21(1), trans_21(2), 0, -trans_21(0),
      -trans_21(1), trans_21(0), 0;
  Mat33_t true_F_21 = cam_matrix_2.transpose().inverse() * skew_21 * rot_21 *
                      cam_matrix_1.inverse();

  // create a fundamental matrix from the rotation and the translation
  const Mat33_t F_21 = solve::fundamental_solver::create_F_21(
      rot_1, trans_1, cam_matrix_1, rot_2, trans_2, cam_matrix_2);
