...
TEST(pnp_solver, without_ransac) {
  ...
  const Mat33_t rot_gt = util::converter::to_rot_mat(
      97.37 * M_PI / 180 * Vec3_t{9.0, -8.5, 1.1}.normalized());
  ...
  auto solver = std::unique_ptr<solve::pnp_solver>(
      new solve::pnp_solver(bearings, keypts, landmarks, scale_factor, 0));
  ...
  const auto rot_err =
      util::converter::to_angle_axis(rot_gt * rot.transpose()).norm();
...
TEST(pnp_solver, without_noise) {
  ...
  // Compute the camera pose by pnp_solver
  auto solver = std::unique_ptr<solve::pnp_solver>(
      new solve::pnp_solver(bearings, keypts, landmarks, scale_factor));
