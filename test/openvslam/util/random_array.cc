...
TEST(random_array, create_random_array_1) {
  ...
  // We are creating a random array with no duplicates, so the minimum value
  // should be 1 and the maximum value should be 5.
...
TEST(random_array, create_random_array_2) {
  ...
  // Since we are creating a random array with no duplicates, the size should
  // not change even if we convert it to std::set
