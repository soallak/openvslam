...
float get_score(data::bow_vocabulary* bow_vocab, const std::string& file1,
...
#ifdef USE_DBOW2
  bow_vocab->transform(util::converter::to_desc_vec(desc1), bow_vec_1,
                       bow_feat_vec_1, 4);
  bow_vocab->transform(util::converter::to_desc_vec(desc2), bow_vec_2,
                       bow_feat_vec_2, 4);
#else
...
TEST(bow_vocabulary, match_near_scene) {
  ...
  const std::string vocab_file_path =
      (vocab_file_path_env != nullptr) ? vocab_file_path_env : "";
  ...
  auto score = get_score(
      bow_vocab, std::string(TEST_DATA_DIR) + "./equirectangular_image_001.jpg",
