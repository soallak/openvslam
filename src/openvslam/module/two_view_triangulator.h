...
class two_view_triangulator {
 public:
  ...
  /**
   * Triangulate a landmark between the keypoint idx_1 of keyfrm_1 and the
   * keypoint idx_2 of keyfrm_2
   */
  bool triangulate(const unsigned idx_1, const unsigned int idx_2,
                   Vec3_t& pos_w) const;

 private:
  /**
   * Check depth is positive or not (if camera model is equirectangular, always
   * return true)
   */
  bool check_depth_is_positive(const Vec3_t& pos_w, const Mat33_t& rot_cw,
   ...
   */
  template <typename T>
  bool check_reprojection_error(const Vec3_t& pos_w, const Mat33_t& rot_cw,
                                const Vec3_t& trans_cw, camera::base* camera,
                                const cv::Point_<T>& keypt, const float x_right,
                                const float sigma_sq,
                                const bool is_stereo) const;

  /**
   * Check estimated and actual scale factors are within the acceptable
   * threshold
   */
  bool check_scale_factors(const Vec3_t& pos_w, const float scale_factor_1,
...
inline bool two_view_triangulator::check_depth_is_positive(
  ...
  return camera->model_type_ == camera::model_type_t::Equirectangular ||
         0 < pos_z;
}

inline bool two_view_triangulator::check_scale_factors(
    const Vec3_t& pos_w, const float scale_factor_1,
    const float scale_factor_2) const {
  ...
  return ratio_octave / ratio_dists < ratio_factor_ &&
         ratio_dists / ratio_octave < ratio_factor_;
}
