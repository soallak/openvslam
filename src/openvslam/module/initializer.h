...
class initializer {
 ...
 private:
  ...
  bool create_map_for_monocular(data::bow_vocabulary* bow_vocab,
                                data::frame& curr_frm);

  //! Scaling up or down a initial map
  void scale_map(const std::shared_ptr<data::keyframe>& init_keyfrm,
  ...
  //! initial matching indices (index: idx of initial frame, value: idx of
  //! current frame)
  ...
  bool create_map_for_stereo(data::bow_vocabulary* bow_vocab,
                             data::frame& curr_frm);
};
