...
two_view_triangulator::two_view_triangulator(
    ...
    : keyfrm_1_(keyfrm_1),
      ...
      ratio_factor_(2.0f * std::max(keyfrm_1->orb_params_->scale_factor_,
                                    keyfrm_2->orb_params_->scale_factor_)),
      cos_rays_parallax_thr_(std::cos(rays_parallax_deg_thr * M_PI / 180.0)) {}

bool two_view_triangulator::triangulate(const unsigned idx_1,
  ...
  const auto cos_stereo_parallax_1 =
      is_stereo_1 ? std::cos(2.0 * atan2(camera_1_->true_baseline_ / 2.0,
                                         keyfrm_1_->frm_obs_.depths_.at(idx_1)))
                  : 2.0;
  const auto cos_stereo_parallax_2 =
      is_stereo_2 ? std::cos(2.0 * atan2(camera_2_->true_baseline_ / 2.0,
                                         keyfrm_2_->frm_obs_.depths_.at(idx_2)))
                  : 2.0;
  const auto cos_stereo_parallax =
      std::min(cos_stereo_parallax_1, cos_stereo_parallax_2);
  ...
  const bool triangulate_with_two_cameras =
      ...
      // check if the parallax between the two cameras is larger than the stereo
      // parallax
      || ((is_stereo_1 || is_stereo_2) && 0.0 < cos_rays_parallax &&
  ...
  if (triangulate_with_two_cameras) {
    pos_w = solve::triangulator::triangulate(ray_c_1, ray_c_2, cam_pose_1w_,
                                             cam_pose_2w_);
  ...
  // reject the point if the real scale factor and the predicted one are much
  // different
  if (!check_scale_factors(
          pos_w, keyfrm_1_->orb_params_->scale_factors_.at(keypt_1.octave),
