...
initializer::initializer(data::map_database* map_db, data::bow_database* bow_db,
    ...
    : map_db_(map_db),
      bow_db_(bow_db),
      num_ransac_iters_(
          yaml_node["num_ransac_iterations"].as<unsigned int>(100)),
      min_num_triangulated_(
          yaml_node["num_min_triangulated_pts"].as<unsigned int>(50)),
...
unsigned int initializer::get_initial_frame_id() const { return init_frm_id_; }
...
void initializer::create_initializer(data::frame& curr_frm) {
  ...
  switch (init_frm_.camera_->model_type_) {
    ...
    case camera::model_type_t::RadialDivision: {
      initializer_ =
          std::unique_ptr<initialize::perspective>(new initialize::perspective(
              init_frm_, num_ransac_iters_, min_num_triangulated_,
...
bool initializer::try_initialize_for_monocular(data::frame& curr_frm) {
  ...
  const auto num_matches = matcher.match_in_consistent_area(
      init_frm_, curr_frm, prev_matched_coords_, init_matches_, 100);
  ...
  assert(initializer_);
  spdlog::debug(
      "try to initialize with the initial frame and the current frame: frame "
      "{} - frame {}",
      init_frm_.id_, curr_frm.id_);
  return initializer_->initialize(curr_frm, init_matches_);
}

bool initializer::create_map_for_monocular(data::bow_vocabulary* bow_vocab,
                                           data::frame& curr_frm) {
  assert(state_ == initializer_state_t::Initializing);
  ...
  for (unsigned int init_idx = 0; init_idx < init_matches_.size(); init_idx++) {
    ...
    auto lm = std::make_shared<data::landmark>(
        init_triangulated_pts.at(init_idx), curr_keyfrm, map_db_);
  ...
  const auto global_bundle_adjuster =
      optimize::global_bundle_adjuster(map_db_, num_ba_iters_, true);
  global_bundle_adjuster.optimize_for_initialization();

  // scale the map so that the median of depths is 1.0
  const auto median_depth =
      init_keyfrm->compute_median_depth(init_keyfrm->camera_->model_type_ ==
  ...
  spdlog::info("new map created with {} points: frame {} - frame {}",
               map_db_->get_num_landmarks(), init_frm_.id_, curr_frm.id_);
  state_ = initializer_state_t::Succeeded;
  return true;
}

void initializer::scale_map(const std::shared_ptr<data::keyframe>& init_keyfrm,
...
bool initializer::try_initialize_for_stereo(data::frame& curr_frm) {
  ...
  unsigned int num_valid_depths = std::count_if(
      curr_frm.frm_obs_.depths_.begin(), curr_frm.frm_obs_.depths_.end(),
      [](const float depth) { return 0 < depth; });
  return min_num_triangulated_ <= num_valid_depths;
}

bool initializer::create_map_for_stereo(data::bow_vocabulary* bow_vocab,
                                        data::frame& curr_frm) {
  assert(state_ == initializer_state_t::Initializing);
  ...
  spdlog::info("new map created with {} points: frame {}",
               map_db_->get_num_landmarks(), curr_frm.id_);
  state_ = initializer_state_t::Succeeded;
  return true;
}
