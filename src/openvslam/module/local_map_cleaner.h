...
class local_map_cleaner {
 public:
   ...
   * Constructor
   */
  explicit local_map_cleaner(data::map_database* map_db,
   ...
   */
  void set_origin_keyframe_id(const unsigned int id) { origin_keyfrm_id_ = id; }
  ...
  unsigned int remove_redundant_landmarks(const unsigned int cur_keyfrm_id,
                                          const bool depth_is_avaliable);

  /**
   * Remove redundant keyframes
   */
  unsigned int remove_redundant_keyframes(
  ...
  void count_redundant_observations(
      ...
      unsigned int& num_valid_obs, unsigned int& num_redundant_obs) const;
