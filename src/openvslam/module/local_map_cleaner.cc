...
local_map_cleaner::local_map_cleaner(data::map_database* map_db,
                                     data::bow_database* bow_db,
                                     double redundant_obs_ratio_thr)
    : map_db_(map_db),
      bow_db_(bow_db),
...
unsigned int local_map_cleaner::remove_redundant_landmarks(
    const unsigned int cur_keyfrm_id, const bool depth_is_avaliable) {
  ...
  while (iter != fresh_landmarks_.end()) {
    ...
    if (lm->will_be_erased()) {
      ...
      // if the number of the observers of `lm` is small after some keyframes
      // were inserted remove `lm` from the buffer and the database
      lm_state = lm_state_t::Invalid;
    } else if (num_reliable_keyfrms + 1U + lm->first_keyfrm_id_ <=
               cur_keyfrm_id) {
      // if the number of the observers of `lm` is sufficient after some
      // keyframes were inserted remove `lm` from the buffer
      lm_state = lm_state_t::Valid;
    }
...
unsigned int local_map_cleaner::remove_redundant_keyframes(
    const std::shared_ptr<data::keyframe>& cur_keyfrm) const {
  ...
  for (const auto& covisibility : cur_covisibilities) {
    ...
    // count the number of redundant observations (num_redundant_obs) and valid
    // observations (num_valid_obs) for the covisibility
    unsigned int num_redundant_obs = 0;
    unsigned int num_valid_obs = 0;
    count_redundant_observations(covisibility, num_valid_obs,
                                 num_redundant_obs);

    // if the redundant observation ratio of `covisibility` is larger than the
    // threshold, it will be removed
    if (redundant_obs_ratio_thr_ <=
        static_cast<float>(num_redundant_obs) / num_valid_obs) {
      ++num_removed;
...
void local_map_cleaner::count_redundant_observations(
    const std::shared_ptr<data::keyframe>& keyfrm, unsigned int& num_valid_obs,
    unsigned int& num_redundant_obs) const {
  // if the number of keyframes that observes the landmark with more reliable
  // scale than the specified keyframe does, it is considered as redundant
  ...
  for (unsigned int idx = 0; idx < landmarks.size(); ++idx) {
    ...
    if (keyfrm->depth_is_avaliable() &&
        (depth < 0.0 || keyfrm->camera_->depth_thr_ < depth)) {
      continue;
    }

    ++num_valid_obs;

    // if the number of the obs is smaller than the threshold, cannot remote the
    // observers
    ...
    // the number of the keyframes that observe `lm` with the more reliable
    // (closer) scale
    ...
    for (const auto& obs : observations) {
      ...
      const auto ngh_scale_level =
          ngh_keyfrm->frm_obs_.undist_keypts_.at(obs.second).octave;
      ...
      if (ngh_scale_level <= scale_level + 1) {
        ...
        if (num_better_obs_thr <= num_better_obs) {
          // if the number of the better observations is greater than the
          // threshold, consider the observation of `lm` by `keyfrm` is
          // redundant
