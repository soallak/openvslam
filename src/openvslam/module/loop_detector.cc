...
loop_detector::loop_detector(data::bow_database* bow_db,
    ...
    : bow_db_(bow_db),
      ...
      num_final_matches_thr_(
          yaml_node["num_final_matches_threshold"].as<unsigned int>(40)),
...
void loop_detector::set_current_keyframe(
...
bool loop_detector::detect_loop_candidates_impl() {
  ...
  if (!loop_detector_is_enabled_ ||
      cur_keyfrm_->id_ < prev_loop_correct_keyfrm_id_ + 10) {
    return false;
  }

  // 1. search loop candidates by inquiring to the BoW dataqbase

  // 1-1. before inquiring, compute the minimum score of BoW similarity between
  // the current and each of the covisibilities
  ...
  // is lower than min_score

  const auto init_loop_candidates =
      bow_db_->acquire_loop_candidates(cur_keyfrm_, min_score);
  ...
  // 2. From now on, we treat each of the candidates as "keyframe set" in order
  // to improve robustness of loop detection
  //    the number of each of the candidate keyframe sets that detected are
  //    counted every time when this member functions is called if the keyframe
  //    sets were detected at the previous call, it is contained in
  //    `cont_detected_keyfrm_sets_` (note that "match of two keyframe sets"
  //    means the intersection of the two sets is NOT empty)

  const auto curr_cont_detected_keyfrm_sets =
      find_continuously_detected_keyframe_sets(cont_detected_keyfrm_sets_,
                                               init_loop_candidates);

  // 3. if the number of the detection is equal of greater than the threshold
  // (`min_continuity_`),
  ...
  for (auto& curr : curr_cont_detected_keyfrm_sets) {
    ...
    // check if the number of the detection is equal of greater than the
    // threshold
...
bool loop_detector::validate_candidates_impl() {
  // 1. for each of the candidates, estimate and validate the Sim3 between it
  // and the current keyframe using the observed landmarks
  //    then, select ONE candaite

  const bool candidate_is_found = select_loop_candidate_via_Sim3(
      loop_candidates_to_validate_, selected_candidate_,
  ...
  // 2. reproject the landmarks observed in covisibilities of the selected
  // candidate to the current keyframe,
  //    then acquire the extra 2D-3D matches

  // matches between the keypoints in the current and the landmarks observed in
  // the covisibilities of the selected candidate
  curr_match_lms_observed_in_cand_covis_.clear();

  auto cand_covisibilities =
      selected_candidate_->graph_node_->get_covisibilities();
  cand_covisibilities.push_back(selected_candidate_);

  // acquire all of the landmarks observed in the covisibilities of the
  // candidate check the already inserted landmarks
  ...
  // reproject the landmarks observed in the covisibilities of the candidate to
  // the current keyframe using Sim3 `Sim3_world_to_curr_`, then, acquire the
  // extra 2D-3D matches however, landmarks in
  // `curr_match_lms_observed_in_cand_` are already matched with keypoints in
  // the current keyframe, thus they are excluded from the reprojection
  ...
  spdlog::debug("acquired {} matches after projection-match",
                num_final_matches);
  ...
  if (num_final_matches_thr_ <= num_final_matches) {
    ...
    spdlog::debug(
        "destruct loop candidate because enough matches not acquired (< {})",
        num_final_matches_thr_);
    return false;
  }
}

float loop_detector::compute_min_score_in_covisibilities(
...
keyframe_sets loop_detector::find_continuously_detected_keyframe_sets(
  ...
  // check the already counted keyframe sets to prevent from counting the same
  // set twice
  ...
  for (const auto& keyfrm_to_search : keyfrms_to_search) {
    ...
    const auto keyfrm_set =
        keyfrm_to_search->graph_node_->get_connected_keyframes();
    ...
    for (const auto& prev : prev_cont_detected_keyfrm_sets) {
      ...
      // compute intersection between the previous set and the current set, then
...
bool loop_detector::select_loop_candidate_via_Sim3(
  ...
  // estimate and the Sim3 between the current keyframe and each of the
  // candidates using the observed landmarks the Sim3 is estimated both in
  // linear and non-linear ways if the inlier after the estimation is lower than
  // the threshold, discard tha candidate
  ...
  for (const auto& candidate : loop_candidates) {
    ...
    // estimate the matches between the keypoints in the current keyframe and
    // the landmarks observed in the candidate
    curr_match_lms_observed_in_cand.clear();
    const auto num_matches = bow_matcher.match_keyframes(
        cur_keyfrm_, candidate, curr_match_lms_observed_in_cand);
    ...
    solve::sim3_solver solver(cur_keyfrm_, candidate,
    ...
    spdlog::debug(
        "found loop candidate via linear Sim3 estimation: keyframe {} - "
        "keyframe {}",
    ...
    g2o::Sim3 g2o_sim3_cand_to_curr(rot_cand_to_curr, trans_cand_to_curr,
                                    scale_cand_to_curr);
    ...
    spdlog::debug(
        "found loop candidate via nonlinear Sim3 optimization: keyframe {} - "
        "keyframe {}",
        candidate->id_, cur_keyfrm_->id_);

    selected_candidate = candidate;
    // convert the estimated Sim3 from "candidate -> current" to "world ->
    // current" this Sim3 indicates the correct camera pose oof the current
    // keyframe after loop correction
...
std::shared_ptr<data::keyframe> loop_detector::get_selected_candidate_keyframe()
    const {
...
std::vector<std::shared_ptr<data::landmark>>
loop_detector::current_matched_landmarks_observed_in_candidate() const {
  return curr_match_lms_observed_in_cand_;
}

std::vector<std::shared_ptr<data::landmark>>
loop_detector::current_matched_landmarks_observed_in_candidate_covisibilities()
    const {
  return curr_match_lms_observed_in_cand_covis_;
}

void loop_detector::set_loop_correct_keyframe_id(
    const unsigned int loop_correct_keyfrm_id) {
