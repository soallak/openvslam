...
typedef std::map<std::shared_ptr<data::keyframe>, g2o::Sim3,
                 std::less<std::shared_ptr<data::keyframe>>,
                 Eigen::aligned_allocator<std::pair<
                     std::shared_ptr<data::keyframe> const, g2o::Sim3>>>
    keyframe_Sim3_pairs_t;

// キーフレームの集合, 中心のキーフレーム, 連続検出回数を合わせた構造体
struct keyframe_set {
  keyframe_set(const std::set<std::shared_ptr<data::keyframe>>& keyfrm_set,
               const std::shared_ptr<data::keyframe>& lead_keyfrm,
               const unsigned int continuity)
      : keyfrm_set_(keyfrm_set),
        lead_keyfrm_(lead_keyfrm),
  ...
  bool intersection_is_empty(
      const std::set<std::shared_ptr<data::keyframe>>& other_set) const {
