...
keyframe_inserter::keyframe_inserter(
    ...
    : max_interval_(max_interval),
      lms_ratio_thr_almost_all_lms_are_tracked_(
...
keyframe_inserter::keyframe_inserter(const YAML::Node& yaml_node)
    : keyframe_inserter(
          yaml_node["max_interval"].as<double>(1.0),
          yaml_node["lms_ratio_thr_almost_all_lms_are_tracked"].as<double>(
              0.95),
...
bool keyframe_inserter::new_keyframe_is_needed(
    data::map_database* map_db, const data::frame& curr_frm,
  ...
  // Count the number of the 3D points that are observed from more than two
  // keyframes
  const unsigned int min_obs_thr = (3 <= num_keyfrms) ? 3 : 2;
  const auto num_reliable_lms =
      ref_keyfrm.get_num_tracked_landmarks(min_obs_thr);
  ...
  // New keyframe is needed if the time elapsed since the last keyframe
  // insertion reaches the threshold
  const bool max_interval_elapsed =
      last_inserted_keyfrm &&
      last_inserted_keyfrm->timestamp_ + max_interval_ <= curr_frm.timestamp_;
  // New keyframe is needed if the field-of-view of the current frame is changed
  // a lot
  const bool view_changed =
      num_tracked_lms < num_reliable_lms * lms_ratio_thr_view_changed_;

  // (Mandatory for keyframe insertion)
  // New keyframe is needed if the number of 3D points exceeds the threshold,
  // and concurrently the ratio of the reliable 3D points larger than the
  // threshold ratio
  bool tracking_is_unstable = num_tracked_lms < num_tracked_lms_thr_unstable;
  bool almost_all_lms_are_tracked =
      num_tracked_lms >
      num_reliable_lms * lms_ratio_thr_almost_all_lms_are_tracked_;

  return (max_interval_elapsed || view_changed) && !tracking_is_unstable &&
         !almost_all_lms_are_tracked && !mapper_is_skipping_localBA;
}

std::shared_ptr<data::keyframe> keyframe_inserter::insert_new_keyframe(
    data::map_database* map_db, data::frame& curr_frm) {
  ...
  // Queue up the keyframe to the mapping module if any valid depth values don't
  // exist
  ...
  for (unsigned int count = 0; count < depth_idx_pairs.size(); ++count) {
    ...
    // Stop adding a keyframe if the number of 3D points exceeds the minimal
    // threshold, and concurrently the depth value exceeds the threshold
    if (min_num_to_create < count && keyfrm->camera_->depth_thr_ < depth) {
      break;
    }

    // Stereo-triangulation cannot be performed if the 3D point has been already
    // associated to the keypoint index
    {
    ...
    // Stereo-triangulation can be performed if the 3D point is not yet
    // associated to the keypoint index
...
void keyframe_inserter::queue_keyframe(
