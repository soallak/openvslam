...
map_publisher::map_publisher(const std::shared_ptr<config>& cfg,
...
unsigned int map_publisher::get_keyframes(
    std::vector<std::shared_ptr<data::keyframe>>& all_keyfrms) {
  all_keyfrms = map_db_->get_all_keyframes();
  return map_db_->get_num_keyframes();
}

unsigned int map_publisher::get_landmarks(
  ...
  local_landmarks = std::set<std::shared_ptr<data::landmark>>(
      _local_landmarks.begin(), _local_landmarks.end());
