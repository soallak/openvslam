...
bool bearing_vector::initialize(const data::frame& cur_frm,
  ...
  // compute an E matrix
  auto essential_solver = solve::essential_solver(
      ref_bearings_, cur_bearings_, ref_cur_matches_, use_fixed_seed_);
...
bool bearing_vector::reconstruct_with_E(
  ...
  // found the most plausible pose from the FOUR hypothesis computed from the E
  // matrix

  // decompose the E matrix
  eigen_alloc_vector<Mat33_t> init_rots;
  eigen_alloc_vector<Vec3_t> init_transes;
  if (!solve::essential_solver::decompose(E_ref_to_cur, init_rots,
                                          init_transes)) {
    return false;
  }
