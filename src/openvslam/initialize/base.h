...
class base {
 public:
  ...
  //! Constructor
  base(const data::frame& ref_frm, const unsigned int num_ransac_iters,
  ...
  //! Get the valid/invalid flags of triangulated 3D points as keypoint indices
  //! in the reference frame
  std::vector<bool> get_triangulated_flags() const;

 protected:
  //! Find the most plausible pose and set them to the member variables
  //! (outputs)
  bool find_most_plausible_pose(const eigen_alloc_vector<Mat33_t>& init_rots,
  ...
  unsigned int check_pose(const Mat33_t& rot_ref_to_cur,
