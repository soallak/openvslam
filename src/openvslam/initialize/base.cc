...
base::base(const data::frame& ref_frm, const unsigned int num_ransac_iters,
    ...
    : ref_camera_(ref_frm.camera_),
      ref_undist_keypts_(ref_frm.frm_obs_.undist_keypts_),
      ref_bearings_(ref_frm.frm_obs_.bearings_),
...
bool base::find_most_plausible_pose(
    const eigen_alloc_vector<Mat33_t>& init_rots,
  ...
  for (unsigned int i = 0; i < num_hypothesis; ++i) {
    nums_valid_pts.at(i) =
        check_pose(init_rots.at(i), init_transes.at(i), is_inlier_match,
                   depth_is_positive, init_triangulated_pts.at(i),
                   init_is_triangulated.at(i), init_parallax.at(i));
  }

  rot_ref_to_cur_ = Mat33_t::Zero();
  trans_ref_to_cur_ = Vec3_t::Zero();

  // find the maximum number of the triangulated 3D points among all of the
  // hypothesis
  ...
  const auto num_similars =
      std::count_if(nums_valid_pts.begin(), nums_valid_pts.end(),
...
unsigned int base::check_pose(const Mat33_t& rot_ref_to_cur,
  ...
  // resize buffers according to the number of observed keypoints in the
  // reference
  ...
  // for each matching, triangulate a 3D point and compute a parallax and a
  // reprojection error
  for (unsigned int i = 0; i < ref_cur_matches_.size(); ++i) {
    ...
    const Vec3_t pos_c_in_ref = solve::triangulator::triangulate(
    ...
    const auto& ref_undist_keypt =
        ref_undist_keypts_.at(ref_cur_matches_.at(i).first);
    const auto& cur_undist_keypt =
        cur_undist_keypts_.at(ref_cur_matches_.at(i).second);
    ...
    const float ref_reproj_err_sq =
        (reproj_in_ref - ref_undist_keypt.pt).squaredNorm();
    ...
    const float cur_reproj_err_sq =
        (reproj_in_cur - cur_undist_keypt.pt).squaredNorm();
