...
unsigned int bow_tree::match_frame_and_keyframe(
    const std::shared_ptr<data::keyframe>& keyfrm, data::frame& frm,
    std::vector<std::shared_ptr<data::landmark>>& matched_lms_in_frm) const {
  unsigned int num_matches = 0;

  angle_checker<int> angle_checker;

  matched_lms_in_frm = std::vector<std::shared_ptr<data::landmark>>(
      frm.frm_obs_.num_keypts_, nullptr);

  const auto keyfrm_lms = keyfrm->get_landmarks();

#ifdef USE_DBOW2
  DBoW2::FeatureVector::const_iterator keyfrm_itr =
      keyfrm->bow_feat_vec_.begin();
  DBoW2::FeatureVector::const_iterator frm_itr = frm.bow_feat_vec_.begin();
  const DBoW2::FeatureVector::const_iterator kryfrm_end =
      keyfrm->bow_feat_vec_.end();
  const DBoW2::FeatureVector::const_iterator frm_end = frm.bow_feat_vec_.end();
#else
  fbow::BoWFeatVector::const_iterator keyfrm_itr =
      keyfrm->bow_feat_vec_.begin();
  fbow::BoWFeatVector::const_iterator frm_itr = frm.bow_feat_vec_.begin();
  const fbow::BoWFeatVector::const_iterator kryfrm_end =
      keyfrm->bow_feat_vec_.end();
    ...
    if (keyfrm_itr->first == frm_itr->first) {
      ...
      for (const auto keyfrm_idx : keyfrm_indices) {
        ...
        for (const auto frm_idx : frm_indices) {
          ...
          const auto hamm_dist =
              compute_descriptor_distance_32(keyfrm_desc, frm_desc);
        ...
        if (check_orientation_) {
          const auto delta_angle =
              keyfrm->frm_obs_.keypts_.at(keyfrm_idx).angle -
              frm.frm_obs_.keypts_.at(best_frm_idx).angle;
      ...
      // Since the node number of the keyframe is smaller, increment the
      // iterator until the node numbers match
      keyfrm_itr = keyfrm->bow_feat_vec_.lower_bound(frm_itr->first);
    } else {
      // Since the node number of the frame is smaller, increment the iterator
...
unsigned int bow_tree::match_keyframes(
  ...
  matched_lms_in_keyfrm_1 = std::vector<std::shared_ptr<data::landmark>>(
      keyfrm_1_lms.size(), nullptr);

  // Set 'true' if a keypoint in keyframe 2 is associated to the keypoint in
  // keyframe 1 NOTE: the size matches the number of the keypoints in keyframe 2
  ...
  while (itr_1 != itr_1_end && itr_2 != itr_2_end) {
    ...
    if (itr_1->first == itr_2->first) {
      ...
      for (const auto idx_1 : keyfrm_1_indices) {
        ...
        // The best index of keyframe 2 already matches the keypoint of keyframe
        // 1
        ...
        if (check_orientation_) {
          const auto delta_angle =
              keyfrm_1->frm_obs_.keypts_.at(idx_1).angle -
              keyfrm_2->frm_obs_.keypts_.at(best_idx_2).angle;
