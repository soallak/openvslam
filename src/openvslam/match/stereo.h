...
class stereo {
 public:
  ...
  stereo(const std::vector<cv::Mat>& left_image_pyramid,
   ...
   */
  void compute(std::vector<float>& stereo_x_right,
               std::vector<float>& depths) const;

 private:
  /**
   * Get the keypoints in the right image which are aligned according to y
   ...
   * @return
   */
  bool compute_subpixel_disparity(const cv::KeyPoint& keypt_left,
