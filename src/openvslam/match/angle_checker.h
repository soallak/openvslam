...
class angle_checker {
 public:
  ...
  /**
   * Get the invalid matches sampled from the remainder of the top-N of the
   * histogram
   */
 ...
 private:
   ...
   */
  std::vector<unsigned int> index_sort_by_size(
...
template <typename T>
angle_checker<T>::angle_checker(const unsigned int histogram_length,
                                const unsigned int num_bins_thr)
...
template <typename T>
inline void angle_checker<T>::append_delta_angle(float delta_angle,
                                                 const T& match) {
  if (delta_angle < 0.0) {
    delta_angle += 360.0;
  }
  ...
  const auto bin =
      static_cast<unsigned int>(cvRound(delta_angle * inv_histogram_length_));
...
std::vector<T> angle_checker<T>::get_valid_matches() const {
  ...
  for (unsigned int bin = 0; bin < histogram_length_; ++bin) {
    const bool is_valid_match =
        std::any_of(bins.begin(), bins.begin() + num_bins_thr_,
    ...
    if (is_valid_match) {
      ...
      valid_matches.reserve(valid_matches.size() +
                            std::distance(bin_begin_iter, bin_end_iter));
...
std::vector<T> angle_checker<T>::get_invalid_matches() const {
  ...
  for (unsigned int bin = 0; bin < histogram_length_; ++bin) {
    ...
    if (!is_valid_match) {
      const auto bin_begin_iter = angle_histogram_.at(bin).cbegin();
      const auto bin_end_iter = angle_histogram_.at(bin).cend();
      invalid_matches.reserve(invalid_matches.size() +
                              std::distance(bin_begin_iter, bin_end_iter));
