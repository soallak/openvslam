...
stereo::stereo(const std::vector<cv::Mat>& left_image_pyramid,
               const std::vector<cv::Mat>& right_image_pyramid,
               const std::vector<cv::KeyPoint>& keypts_left,
               const std::vector<cv::KeyPoint>& keypts_right,
               const cv::Mat& descs_left, const cv::Mat& descs_right,
               const std::vector<float>& scale_factors,
               const std::vector<float>& inv_scale_factors,
...
void stereo::compute(std::vector<float>& stereo_x_right,
  ...
  // Compute the parallax and depth for each keypoint on the left image in a
  // subpixel precision
...
#pragma omp parallel for
  ...
  for (unsigned int idx_left = 0; idx_left < num_keypts_; ++idx_left) {
    ...
    // Acquire the index of the keypoint on the right image which is observed at
    // the same height level of the one on the left image This is candidate
    // matching
    ...
    // Search the best candidate index on the right image whose feature vector
    // is the closest to that on the left
    unsigned int best_idx_right = 0;
    unsigned int best_hamm_dist = hamm_dist_thr_;
    find_closest_keypoints_in_stereo(
        idx_left, scale_level_left, candidate_indices_right, min_x_right,
        max_x_right, best_idx_right, best_hamm_dist);
...
#pragma omp critical
#endif
    {
      correlation_and_idx_left.emplace_back(
          std::make_pair(best_correlation, idx_left));
    }
  }
  ...
  // Discard if the correlation is weaker than the double median value of the
  // correlation
  const float correlation_thr = 2.0 * median_correlation;

  // Start with 'median_i' because the threshold is set to the double median
...
stereo::get_right_keypoint_indices_in_each_row(const float margin) const {
  // Save keypoint indices on the right image in each image row
  ...
  for (unsigned int idx_right = 0; idx_right < num_keypts_right; ++idx_right) {
    ...
    const float r =
        margin * scale_factors_.at(keypts_right_.at(idx_right).octave);
    // Compute the max and the min values
    const int max_r = cvCeil(y_right + r);
    const int min_r = cvFloor(y_right - r);

    // Save the index of the keypoint for all the row numbers between the max
    // and the min values
...
void stereo::find_closest_keypoints_in_stereo(
  ...
  // Compute each hamming distance between the keypoints on the right and left
  // images For each of the keypoints on the left image, acquire the index of
  // the closest keypoint on the right image
  for (const auto idx_right : candidate_indices_right) {
    const auto& keypt_right = keypts_right_.at(idx_right);
    // Discard if the ORB scale becomes significantly different
    if (keypt_right.octave < scale_level_left - 1 ||
        keypt_right.octave > scale_level_left + 1) {
      continue;
    }
    ...
    const unsigned int hamm_dist =
        match::compute_descriptor_distance_32(desc_left, desc_right);
...
bool stereo::compute_subpixel_disparity(const cv::KeyPoint& keypt_left,
                                        const cv::KeyPoint& keypt_right,
                                        float& best_x_right, float& best_disp,
                                        float& best_correlation) const {
  // The keypoint on the right image whose hamming distance is cloest
  const float x_right = keypt_right.pt.x;
  // Convert cordinates to multiple scaling to compute patch correlation on the
  // scaled image
  ...
  // Compute the pixel correlation surrounding the keypoint, and compute the
  // parallax in subpixel precision by parabolic fitting
  ...
  auto patch_left =
      left_image_pyramid_.at(keypt_left.octave)
  ...
  for (int offset = -slide_width; offset <= +slide_width; ++offset) {
    ...
    auto patch_right =
        right_image_pyramid_.at(keypt_left.octave)
            .rowRange(scaled_y_left - win_size, scaled_y_left + win_size + 1)
            .colRange(scaled_x_right + offset - win_size,
                      scaled_x_right + offset + win_size + 1);
  ...
  // Apply parabolic fitting to the three-point correlation value centering the
  // point with the strongest correlation
  ...
  // Parabolic vertex coordinates passing through the three points: x_delta =
  // (-1, correlation_1), (0, correlation_2), (+1, correlation_1)
  const float x_delta =
      (correlation_1 - correlation_3) /
      (2.0 * (correlation_1 + correlation_3) - 4.0 * correlation_2);
  ...
  best_x_right = scale_factors_.at(keypt_left.octave) *
                 (scaled_x_right + best_offset + x_delta);
