...
class projection final : public base {
 public:
  explicit projection(const float lowe_ratio = 0.6,
  ...
  //! last frameで観測している3次元点をcurrent
  //! frameに再投影し，frame.landmarks_に対応情報を記録する
  unsigned int match_current_and_last_frames(data::frame& curr_frm,
                                             const data::frame& last_frm,
                                             const float margin) const;

  //! keyfarmeで観測している3次元点をcurrent
  //! frameに再投影し，frame.landmarks_に対応情報を記録する current
  //! frameとすでに対応が取れているものは，already_matched_lmsに指定して再投影しないようにする
