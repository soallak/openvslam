...
unsigned int robust::match_for_triangulation(
    ...
    const std::shared_ptr<data::keyframe>& keyfrm_2, const Mat33_t& E_12,
  ...
  const bool valid_epiplane = keyfrm_2->camera_->reproject_to_bearing(
      rot_2w, trans_2w, cam_center_1, epiplane_in_keyfrm_2);
  ...
  std::vector<bool> is_already_matched_in_keyfrm_2(
      keyfrm_2->frm_obs_.num_keypts_, false);
  // Save the keypoint idx in keyframe 2 which is already associated to the
  // keypoint idx in keyframe 1
  std::vector<int> matched_indices_2_in_keyfrm_1(keyfrm_1->frm_obs_.num_keypts_,
                                                 -1);
  ...
  const DBoW2::FeatureVector::const_iterator itr_1_end =
      keyfrm_1->bow_feat_vec_.end();
  const DBoW2::FeatureVector::const_iterator itr_2_end =
      keyfrm_2->bow_feat_vec_.end();
#else
  ...
  const fbow::BoWFeatVector::const_iterator itr_1_end =
      keyfrm_1->bow_feat_vec_.end();
  const fbow::BoWFeatVector::const_iterator itr_2_end =
      keyfrm_2->bow_feat_vec_.end();
#endif
  ...
  while (itr_1 != itr_1_end && itr_2 != itr_2_end) {
    ...
    if (itr_1->first == itr_2->first) {
      ...
      for (const auto idx_1 : keyfrm_1_indices) {
        ...
        const bool is_stereo_keypt_1 =
            0 <= keyfrm_1->frm_obs_.stereo_x_right_.at(idx_1);
        ...
        for (const auto idx_2 : keyfrm_2_indices) {
          ...
          // Check if it's a stereo keypoint or not
          const bool is_stereo_keypt_2 =
              0 <= keyfrm_2->frm_obs_.stereo_x_right_.at(idx_2);
          ...
          if (valid_epiplane && !is_stereo_keypt_1 && !is_stereo_keypt_2) {
            // Do not use any keypoints near the epipole if both are not stereo
            // keypoints
            const auto cos_dist = epiplane_in_keyfrm_2.dot(bearing_2);
            // The threshold of the minimum angle formed by the epipole and the
            // bearing vector is 3.0 degree
            constexpr double cos_dist_thr = 0.99862953475;

            // Do not allow to match if the formed angle is narrower that the
            // threshold value
        ...
        if (check_orientation_) {
          const auto delta_angle =
              keypt_1.angle -
              keyfrm_2->frm_obs_.undist_keypts_.at(best_idx_2).angle;
      ...
      // Since the node number of keyframe 1 is smaller, increment the iterator
      // until the node numbers match
      itr_1 = keyfrm_1->bow_feat_vec_.lower_bound(itr_2->first);
    } else {
      // Since the node number of keyframe 2 is smaller, increment the iterator
      // until the node numbers match
  ...
  for (unsigned int idx_1 = 0; idx_1 < matched_indices_2_in_keyfrm_1.size();
       ++idx_1) {
    if (matched_indices_2_in_keyfrm_1.at(idx_1) < 0) {
      continue;
    }
    matched_idx_pairs.emplace_back(
        std::make_pair(idx_1, matched_indices_2_in_keyfrm_1.at(idx_1)));
  }
...
unsigned int robust::match_frame_and_keyframe(
    data::frame& frm, const std::shared_ptr<data::keyframe>& keyfrm,
  ...
  matched_lms_in_frm =
      std::vector<std::shared_ptr<data::landmark>>(num_frm_keypts, nullptr);
  ...
  solve::essential_solver solver(frm.frm_obs_.bearings_,
                                 keyfrm->frm_obs_.bearings_, matches);
...
unsigned int robust::brute_force_match(
  ...
  // 2. Acquire ORB descriptors in the keyframe which are the first and second
  // closest to the descriptors in the frame
  ...
  for (unsigned int idx_2 = 0; idx_2 < num_keypts_2; ++idx_2) {
    ...
    // Acquire the descriptors in the frame which are the first and second
    // closest to the descriptor in the keyframe
    ...
    // Ratio test
    if (lowe_ratio_ * second_best_hamm_dist <
        static_cast<float>(best_hamm_dist)) {
      continue;
    }
...
bool robust::check_epipolar_constraint(
  ...
  // (e.g. for the camera with width of 900-pixel and 90-degree FOV, 0.2 degree
  // is equivalent to 2 pixel in the horizontal direction)
