...
unsigned int projection::match_frame_and_landmarks(
  ...
  for (auto local_lm : local_landmarks) {
    ...
    for (const auto idx : indices_in_cell) {
      ...
      if (0 < frm.frm_obs_.stereo_x_right_.at(idx)) {
        const auto reproj_error =
            std::abs(lm_to_x_right.at(local_lm->id_) -
                     frm.frm_obs_.stereo_x_right_.at(idx));
        if (margin * frm.orb_params_->scale_factors_.at(pred_scale_level) <
            reproj_error) {
          continue;
        }
      }
    ...
    if (best_hamm_dist <= HAMMING_DIST_THR_HIGH) {
      // Lowe's ratio test
      if (best_scale_level == second_best_scale_level &&
...
unsigned int projection::match_current_and_last_frames(
  ...
  // For non-monocular, check if the z component of the current-to-last
  // translation vector is moving forward The z component is positive going ->
  // moving forward
  ...
  for (unsigned int idx_last = 0; idx_last < last_frm.frm_obs_.num_keypts_;
       ++idx_last) {
    auto& lm = last_frm.landmarks_.at(idx_last);
    if (!lm) {
      continue;
    }
    // Discard any matches that were marked as outliers after applyng pose
    // optimization
    ...
    Vec2_t reproj;
    float x_right;
    const bool in_image = curr_frm.camera_->reproject_to_image(
    ...
    auto indices = curr_frm.get_keypoints_in_cell(
        reproj(0), reproj(1),
    ...
    for (const auto curr_idx : indices) {
      if (curr_frm.landmarks_.at(curr_idx) &&
          curr_frm.landmarks_[curr_idx]->has_observation()) {
        continue;
      }

      if (curr_frm.frm_obs_.stereo_x_right_.at(curr_idx) > 0) {
        const float reproj_error =
            std::fabs(x_right - curr_frm.frm_obs_.stereo_x_right_.at(curr_idx));
        if (margin * curr_frm.orb_params_->scale_factors_.at(last_scale_level) <
            reproj_error) {
          continue;
        }
      }
    ...
    if (check_orientation_) {
      const auto delta_angle =
          last_frm.frm_obs_.undist_keypts_.at(idx_last).angle -
          curr_frm.frm_obs_.undist_keypts_.at(best_idx).angle;
...
unsigned int projection::match_frame_and_keyframe(
    data::frame& curr_frm, const std::shared_ptr<data::keyframe>& keyfrm,
  ...
  for (unsigned int idx = 0; idx < landmarks.size(); idx++) {
    ...
    if (cam_to_lm_dist < min_cam_to_lm_dist ||
        max_cam_to_lm_dist < cam_to_lm_dist) {
      continue;
    }

    // Acquire keypoints in the cell where the reprojected 3D points exist
    const auto pred_scale_level = lm->predict_scale_level(
        cam_to_lm_dist, curr_frm.orb_params_->num_levels_,
        curr_frm.orb_params_->log_scale_factor_);
    ...
    if (check_orientation_) {
      const auto delta_angle =
          keyfrm->frm_obs_.undist_keypts_.at(idx).angle -
          curr_frm.frm_obs_.undist_keypts_.at(best_idx).angle;
      angle_checker.append_delta_angle(delta_angle, best_idx);
    }
  }
...
unsigned int projection::match_by_Sim3_transform(
    const std::shared_ptr<data::keyframe>& keyfrm, const Mat44_t& Sim3_cw,
    const std::vector<std::shared_ptr<data::landmark>>& landmarks,
    std::vector<std::shared_ptr<data::landmark>>& matched_lms_in_keyfrm,
    const float margin) const {
  unsigned int num_matches = 0;
  ...
  const auto s_cw =
      std::sqrt(s_rot_cw.block<1, 3>(0, 0).dot(s_rot_cw.block<1, 3>(0, 0)));
  const Mat33_t rot_cw = s_rot_cw / s_cw;
  const Vec3_t trans_cw = Sim3_cw.block<3, 1>(0, 3) / s_cw;
  const Vec3_t cam_center = -rot_cw.transpose() * trans_cw;

  std::set<std::shared_ptr<data::landmark>> already_matched(
  ...
  for (const auto& lm : landmarks) {
    ...
    Vec2_t reproj;
    float x_right;
    const bool in_image = keyfrm->camera_->reproject_to_image(
        rot_cw, trans_cw, pos_w, reproj, x_right);
    ...
    // Compute the angle formed by the average observation vector of the 3D
    // points, and discard it if it is wider than the threshold value (60
    // degrees)
    ...
    const auto pred_scale_level = lm->predict_scale_level(
        ...
        keyfrm->orb_params_->log_scale_factor_);
    const auto indices = keyfrm->get_keypoints_in_cell(
        reproj(0), reproj(1),
        margin * keyfrm->orb_params_->scale_factors_.at(pred_scale_level));
    ...
    for (const auto idx : indices) {
      ...
      const auto scale_level =
          static_cast<unsigned int>(keyfrm->frm_obs_.keypts_.at(idx).octave);

      // TODO: should determine the scale with 'keyfrm-> get_keypts_in_cell ()'
      if (scale_level < pred_scale_level - 1 ||
          pred_scale_level < scale_level) {
        continue;
      }
...
unsigned int projection::match_keyframes_mutually(
    ...
    const float& s_12, const Mat33_t& rot_12, const Vec3_t& trans_12,
    const float margin) const {
  ...
  // Contain matching information if there are already matches between the
  // keyframes 1 and 2
  ...
  // Compute the similarity transformation from the 3D points observed in
  // keyframe 1 to keyframe 2 coordinates, then project the result, and search
  // keypoint matches (world origin -- SE3 -> keyframe 1 -- Sim3 --> keyframe 2)
  ...
  // = s_rot_21 * rot_1w * pos_w + s_rot_21 * trans_1w + trans_21
  {
    ...
    for (unsigned int idx_1 = 0; idx_1 < landmarks_1.size(); ++idx_1) {
      ...
      Vec2_t reproj;
      float x_right;
      const bool in_image = keyfrm_2->camera_->reproject_to_image(
          s_rot_21w, trans_21w, pos_w, reproj, x_right);
      ...
      const auto pred_scale_level = lm->predict_scale_level(
          ...
          keyfrm_2->orb_params_->log_scale_factor_);
      const auto indices = keyfrm_2->get_keypoints_in_cell(
          reproj(0), reproj(1),
          margin * keyfrm_2->orb_params_->scale_factors_.at(pred_scale_level));
      ...
      for (const auto idx_2 : indices) {
        const auto scale_level = static_cast<unsigned int>(
            keyfrm_2->frm_obs_.keypts_.at(idx_2).octave);
  ...
  // Compute the similarity transformation from the 3D points observed in the
  // current keyframe (keyframe 1) to the candidate keyframe (keyframe 2)
  // coordinates, then project the result earch keypoint matches (world origin
  // -- SE3 -> keyframe2 -- Sim3 --> keyframe1) s_rot_12 * (rot_2w * pos_w +
  // trans_2w) + trans_12 = s_rot_12 * rot_2w * pos_w + s_rot_12 * trans_2w +
  // trans_12
  {
    ...
    for (unsigned int idx_2 = 0; idx_2 < landmarks_2.size(); ++idx_2) {
      ...
      float x_right;
      const bool in_image = keyfrm_2->camera_->reproject_to_image(
          s_rot_12w, trans_12w, pos_w, reproj, x_right);
      ...
      const auto pred_scale_level = lm->predict_scale_level(
          ...
          keyfrm_1->orb_params_->log_scale_factor_);

      const auto indices = keyfrm_1->get_keypoints_in_cell(
          reproj(0), reproj(1),
          margin * keyfrm_1->orb_params_->scale_factors_.at(pred_scale_level));
      ...
      for (const auto idx_1 : indices) {
        const auto scale_level = static_cast<unsigned int>(
            keyfrm_1->frm_obs_.keypts_.at(idx_1).octave);
