...
class tracking_module {
 public:
  ...
  //! Constructor
  tracking_module(const std::shared_ptr<config>& cfg,
                  data::map_database* map_db, data::bow_vocabulary* bow_vocab,
  ...
  bool request_relocalize_by_pose_2d(const Mat44_t& pose,
  ...
  //! closest keyframes thresholds (by distance and angle) to relocalize with
  //! when updating by pose
 ...
 protected:
  ...
  //! Get close keyframes
  std::vector<std::shared_ptr<data::keyframe>> get_close_keyframes(
      const pose_request& request);
