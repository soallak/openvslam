...
orb_params_database::~orb_params_database() {
  for (const auto& name_orb_params : database_) {
    ...
    // Since curr_orb_params is held in the config class, do not delete
    // curr_orb_params here
...
feature::orb_params* orb_params_database::get_orb_params(
    const std::string& orb_params_name) const {
...
void orb_params_database::from_json(const nlohmann::json& json_orb_params) {
  ...
  for (const auto& json_id_orb_params : json_orb_params.items()) {
    ...
    if (orb_params_name == curr_orb_params_->name_) {
      if (std::abs(curr_orb_params_->scale_factor_ -
                   json_orb_params.at("scale_factor").get<float>()) < 1e-6 &&
          curr_orb_params_->num_levels_ -
                  json_orb_params.at("num_levels").get<unsigned int>() ==
              0 &&
          curr_orb_params_->ini_fast_thr_ -
                  json_orb_params.at("ini_fast_threshold")
                      .get<unsigned int>() ==
              0 &&
          curr_orb_params_->min_fast_thr_ -
                  json_orb_params.at("min_fast_threshold")
                      .get<unsigned int>() ==
              0) {
        ...
        throw std::runtime_error(
            ...
            "Please give them different names.");
      }
    }

    spdlog::info("load a orb_params \"{}\" from JSON", orb_params_name);

    auto orb_params = new feature::orb_params(
        orb_params_name, json_orb_params.at("scale_factor").get<float>(),
        json_orb_params.at("num_levels").get<unsigned int>(),
