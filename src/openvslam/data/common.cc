...
std::vector<cv::KeyPoint> convert_json_to_keypoints(
  ...
  for (unsigned int idx = 0; idx < json_keypts.size(); ++idx) {
    ...
    keypts.at(idx) = cv::KeyPoint(json_keypt.at("pt").at(0).get<float>(),
                                  json_keypt.at("pt").at(1).get<float>(), 0,
                                  json_keypt.at("ang").get<float>(), 0,
                                  json_keypt.at("oct").get<unsigned int>(), -1);
  }
  return keypts;
}
...
nlohmann::json convert_undistorted_to_json(
  ...
  for (unsigned int idx = 0; idx < undist_keypts.size(); ++idx) {
    json_undist_keypts.at(idx) = {undist_keypts.at(idx).pt.x,
                                  undist_keypts.at(idx).pt.y};
  }
...
std::vector<cv::KeyPoint> convert_json_to_undistorted(
    ...
    const std::vector<cv::KeyPoint>& keypts) {
  auto undist_keypts =
      (keypts.empty() ? std::vector<cv::KeyPoint>(json_undist_keypts.size())
                      : keypts);
...
std::vector<unsigned int> get_keypoints_in_cell(
  ...
  const int min_cell_idx_x =
      std::max(0, cvFloor((ref_x - camera->img_bounds_.min_x_ - margin) *
                          camera->inv_cell_width_));
  if (static_cast<int>(camera->num_grid_cols_) <= min_cell_idx_x) {
    return indices;
  }

  const int max_cell_idx_x =
      std::min(static_cast<int>(camera->num_grid_cols_ - 1),
               cvCeil((ref_x - camera->img_bounds_.min_x_ + margin) *
                      camera->inv_cell_width_));
  if (max_cell_idx_x < 0) {
    return indices;
  }
  ...
  const int min_cell_idx_y =
      std::max(0, cvFloor((ref_y - camera->img_bounds_.min_y_ - margin) *
                          camera->inv_cell_height_));
  if (static_cast<int>(camera->num_grid_rows_) <= min_cell_idx_y) {
    return indices;
  }

  const int max_cell_idx_y =
      std::min(static_cast<int>(camera->num_grid_rows_ - 1),
               cvCeil((ref_y - camera->img_bounds_.min_y_ + margin) *
                      camera->inv_cell_height_));
  if (max_cell_idx_y < 0) {
    return indices;
  }
  ...
  for (int cell_idx_x = min_cell_idx_x; cell_idx_x <= max_cell_idx_x;
       ++cell_idx_x) {
    for (int cell_idx_y = min_cell_idx_y; cell_idx_y <= max_cell_idx_y;
         ++cell_idx_y) {
      const auto& keypt_indices_in_cell =
          keypt_indices_in_cells.at(cell_idx_x).at(cell_idx_y);
