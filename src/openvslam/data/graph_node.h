...
class graph_node {
 public:
  ...
  explicit graph_node(std::shared_ptr<keyframe>& keyfrm,
                      const bool spanning_parent_is_not_set = true);
   ...
   */
  void add_connection(const std::shared_ptr<keyframe>& keyfrm,
  ...
  /**
   * Update the connections and the covisibilities by referring landmark
   * observations
   */
   ...
   */
  std::vector<std::shared_ptr<keyframe>> get_top_n_covisibilities(
      const unsigned int num_covisibilities) const;

  /**
   * Get the covisibility keyframes which have weights over the threshold with
   * myself
   */
  std::vector<std::shared_ptr<keyframe>> get_covisibilities_over_weight(
 ...
 private:
  ...
  static std::vector<std::shared_ptr<keyframe>> extract_intersection(
