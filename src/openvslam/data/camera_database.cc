...
camera_database::~camera_database() {
  for (const auto& name_camera : database_) {
    ...
    // Since curr_camera is held in the config class, do not delete curr_camera
    // here
...
camera::base* camera_database::get_camera(
...
void camera_database::from_json(const nlohmann::json& json_cameras) {
  ...
  for (const auto& json_id_camera : json_cameras.items()) {
    ...
    const auto setup_type = camera::base::load_setup_type(
        json_camera.at("setup_type").get<std::string>());
    const auto model_type = camera::base::load_model_type(
        json_camera.at("model_type").get<std::string>());
    const auto color_order = camera::base::load_color_order(
        json_camera.at("color_order").get<std::string>());

    switch (model_type) {
      case camera::model_type_t::Perspective: {
        camera = new camera::perspective(
            camera_name, setup_type, color_order,
      ...
      case camera::model_type_t::RadialDivision: {
        camera = new camera::radial_division(
            camera_name, setup_type, color_order,
            json_camera.at("cols").get<unsigned int>(),
            json_camera.at("rows").get<unsigned int>(),
            json_camera.at("fps").get<double>(),
