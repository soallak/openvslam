...
class landmark {
 public:
  ...
  //! constructor for map loading with computing parameters which can be
  //! recomputed
  ...
  //! get mean normalized vector of keyframe->lm vectors, for keyframes such
  //! that observe the 3D point.
  ...
  //! add observation
  void add_observation(const std::shared_ptr<keyframe>& keyfrm,
                       unsigned int idx);
  //! erase observation
  void erase_observation(map_database* map_db,
  ...
  //! predict scale level assuming this landmark is observed in the specified
  //! frame/keyframe
  unsigned int predict_scale_level(const float cam_to_lm_dist,
 ...
 private:
  ...
  //! Normalized average vector (unit vector) of keyframe->lm, for keyframes
  //! such that observe the 3D point.
