...
double get_reloc_distance_threshold(const YAML::Node& yaml_node) {
  spdlog::debug(
      "load maximum distance threshold where close keyframes could be found");
  return yaml_node["reloc_distance_threshold"].as<double>(0.2);
}

double get_reloc_angle_threshold(const YAML::Node& yaml_node) {
  spdlog::debug(
      "load maximum angle threshold between given pose and close keyframes");
...
double get_use_robust_matcher_for_relocalization_request(
    const YAML::Node& yaml_node) {
  return yaml_node["use_robust_matcher_for_relocalization_request"].as<bool>(
      false);
}
...
tracking_module::tracking_module(const std::shared_ptr<config>& cfg,
                                 data::map_database* map_db,
                                 data::bow_vocabulary* bow_vocab,
                                 data::bow_database* bow_db)
    : camera_(cfg->camera_),
      reloc_distance_threshold_(get_reloc_distance_threshold(
          util::yaml_optional_ref(cfg->yaml_node_, "Tracking"))),
      reloc_angle_threshold_(get_reloc_angle_threshold(
          util::yaml_optional_ref(cfg->yaml_node_, "Tracking"))),
      enable_auto_relocalization_(get_enable_auto_relocalization(
          util::yaml_optional_ref(cfg->yaml_node_, "Tracking"))),
      use_robust_matcher_for_relocalization_request_(
          get_use_robust_matcher_for_relocalization_request(
              util::yaml_optional_ref(cfg->yaml_node_, "Tracking"))),
      map_db_(map_db),
      bow_vocab_(bow_vocab),
      bow_db_(bow_db),
      initializer_(map_db, bow_db,
                   util::yaml_optional_ref(cfg->yaml_node_, "Initializer")),
      frame_tracker_(camera_, 10),
      relocalizer_(util::yaml_optional_ref(cfg->yaml_node_, "Relocalizer")),
      pose_optimizer_(),
      keyfrm_inserter_(
          util::yaml_optional_ref(cfg->yaml_node_, "KeyframeInserter")) {
...
void tracking_module::set_global_optimization_module(
...
bool tracking_module::request_relocalize_by_pose(const Mat44_t& pose) {
  ...
  if (relocalize_by_pose_is_requested_) {
    spdlog::warn(
        "Can not process new pose update request while previous was not "
        "finished");
    return false;
  }
...
bool tracking_module::request_relocalize_by_pose_2d(
...
std::shared_ptr<Mat44_t> tracking_module::feed_frame(data::frame curr_frm) {
  ...
  if (succeeded) {
    ...
    // if tracking is failed within 5.0 sec after initialization, reset the
    // system
    constexpr float init_retry_thr = 5.0;
    if (curr_frm_.timestamp_ - initializer_.get_initial_frame_timestamp() <
        init_retry_thr) {
      spdlog::info("tracking lost within {} sec after initialization",
                   init_retry_thr);
      reset();
      return nullptr;
    }
  ...
  if (curr_frm_.cam_pose_cw_is_valid_) {
    ...
    cam_pose_wc = std::allocate_shared<Mat44_t>(
        Eigen::aligned_allocator<Mat44_t>(), curr_frm_.get_cam_pose_inv());
  }
...
bool tracking_module::track(bool relocalization_is_needed) {
  ...
  // because the mapping module might optimize the camera pose of the last
  // frame's reference keyframe
  update_last_frame();
  ...
  if (succeeded) {
    ...
    succeeded =
        optimize_current_frame_with_local_map(num_tracked_lms, outlier_ids);
  }
...
bool tracking_module::track_current_frame(
  ...
  if (!succeeded) {
    ...
    succeeded = frame_tracker_.bow_match_based_track(
        curr_frm_, last_frm_, curr_frm_.ref_keyfrm_, outlier_ids);
  }
  if (!succeeded) {
    succeeded = frame_tracker_.robust_match_based_track(
        curr_frm_, last_frm_, curr_frm_.ref_keyfrm_, outlier_ids);
  }
...
bool tracking_module::relocalize_by_pose(const pose_request& request) {
  ...
  if (!candidates.empty()) {
    succeeded = relocalizer_.reloc_by_candidates(
        curr_frm_, candidates, use_robust_matcher_for_relocalization_request_);
    if (succeeded) {
      last_reloc_frm_id_ = curr_frm_.id_;
      last_reloc_frm_timestamp_ = curr_frm_.timestamp_;
    }
...
tracking_module::get_close_keyframes(const pose_request& request) {
  if (request.mode_2d_) {
    return map_db_->get_close_keyframes_2d(
        request.pose_, request.normal_vector_, reloc_distance_threshold_,
        reloc_angle_threshold_);
  } else {
    return map_db_->get_close_keyframes(
        request.pose_, reloc_distance_threshold_, reloc_angle_threshold_);
  }
}
...
void tracking_module::update_last_frame() {
  ...
  last_frm_.set_cam_pose(last_cam_pose_from_ref_keyfrm_ *
                         last_ref_keyfrm->get_cam_pose());
}

bool tracking_module::optimize_current_frame_with_local_map(
    unsigned int& num_tracked_lms,
    std::unordered_set<unsigned int>& outlier_ids) {
  // acquire more 2D-3D matches by reprojecting the local landmarks to the
  // current frame
  ...
  if (curr_frm_.timestamp_ < last_reloc_frm_timestamp_ + 1.0 &&
      num_tracked_lms < 2 * num_tracked_lms_thr) {
    spdlog::debug("local map tracking failed: {} matches < {}", num_tracked_lms,
                  2 * num_tracked_lms_thr);
    return false;
  }
...
void tracking_module::update_local_map(
  ...
  auto local_map_updater =
      module::local_map_updater(curr_frm_, max_num_local_keyfrms);
...
void tracking_module::search_local_landmarks(
    std::unordered_set<unsigned int>& outlier_ids) {
  // select the landmarks which can be reprojected from the ones observed in the
  // current frame
  ...
  // acquire more 2D-3D matches by projecting the local landmarks to the current
  // frame
  match::projection projection_matcher(0.8);
  const float margin =
      (curr_frm_.id_ < last_reloc_frm_id_ + 2)
          ? 20.0
  ...
  projection_matcher.match_frame_and_landmarks(curr_frm_, local_landmarks_,
                                               lm_to_reproj, lm_to_x_right,
                                               lm_to_scale, margin);
}

bool tracking_module::new_keyframe_is_needed(
    unsigned int num_tracked_lms) const {
  ...
  return keyfrm_inserter_.new_keyframe_is_needed(
      map_db_, curr_frm_, num_tracked_lms, *curr_frm_.ref_keyfrm_);
}

void tracking_module::insert_new_keyframe() {
  // insert the new keyframe
  const auto ref_keyfrm =
