...
pnp_solver::~pnp_solver() { spdlog::debug("DESTRUCT: solve::pnp_solver"); }

void pnp_solver::find_via_ransac(const unsigned int max_num_iter,
  ...
  for (unsigned int iter = 0; iter < max_num_iter; ++iter) {
    ...
    const auto random_indices = util::create_random_array(
        min_set_size, 0U, num_matches_ - 1, random_engine_);
    ...
    compute_pose(min_set_bearings, min_set_pos_ws, rot_cw_in_sac,
                 trans_cw_in_sac);

    // 2-3. Check inliers and compute a score
    const auto num_inliers =
        check_inliers(rot_cw_in_sac, trans_cw_in_sac, is_inlier_match_in_sac);
...
unsigned int pnp_solver::check_inliers(const Mat33_t& rot_cw,
  ...
  for (unsigned int i = 0; i < num_matches_; ++i) {
    ...
    // Compute cosine similarity between the bearing vector and the position of
    // the 3D point
    const auto cos = pos_c.dot(bearing) / pos_c.norm();

    // The match is inlier if the cosine similarity is less than or equal to the
    // threshold
...
double pnp_solver::compute_pose(
    const eigen_alloc_vector<Vec3_t>& bearing_vectors,
    const eigen_alloc_vector<Vec3_t>& pos_ws, Mat33_t& rot_cw, Vec3_t& trans_cw,
    const unsigned int num_iter) {
  // EPnP: An AccurateO(n)Solution to the PnP Problem
  // (Lepetit et al. in IJCV 2009)

  const eigen_alloc_vector<Vec3_t> control_points =
      choose_control_points(pos_ws);
  const eigen_alloc_vector<Vec4_t> alphas =
      compute_barycentric_coordinates(control_points, pos_ws);
  ...
  Eigen::JacobiSVD<MatRC_t<12, 12>> SVD(
  ...
  for (unsigned int N = 2; N <= 4; N++) {
    ...
    const eigen_alloc_vector<Vec3_t> pcs =
        compute_pcs(alphas, ccs, bearing_z_sign);

    estimate_R_and_t(pos_ws, pcs, rot_cand, trans_cand);

    const auto reproj_error =
        reprojection_error(pos_ws, bearing_vectors, rot_cand, trans_cand);

    // Take the rotation and translation which archieve the minimum reprojection
    // error
...
eigen_alloc_vector<Vec3_t> pnp_solver::choose_control_points(
  ...
  Eigen::JacobiSVD<MatX_t> SVD(PW0tPW0,
...
eigen_alloc_vector<Vec4_t> pnp_solver::compute_barycentric_coordinates(
    const eigen_alloc_vector<Vec3_t>& control_points,
    const eigen_alloc_vector<Vec3_t>& pos_ws) {
  const unsigned int num_correspondences = pos_ws.size();
...
eigen_alloc_vector<Vec3_t> pnp_solver::compute_ccs(const Vec4_t& betas,
                                                   const MatX_t& U) {
...
eigen_alloc_vector<Vec3_t> pnp_solver::compute_pcs(
    const eigen_alloc_vector<Vec4_t>& alphas,
    const eigen_alloc_vector<Vec3_t>& ccs, const bool bearing_z_sign) {
  const unsigned int num_correspondences = alphas.size();
  eigen_alloc_vector<Vec3_t> pcs;
  // Compute local 3D points using the barycentric coordinates and the local
  // control points
  for (unsigned int i = 0; i < num_correspondences; ++i) {
    const Vec4_t a = alphas.at(i);
    pcs.emplace_back(a(0) * ccs.at(0) + a(1) * ccs.at(1) + a(2) * ccs.at(2) +
                     a(3) * ccs.at(3));
  }
  // Invert the local points if they have not the same direction of the bearing
  // vectors
...
double pnp_solver::reprojection_error(
    const eigen_alloc_vector<Vec3_t>& pws,
    const eigen_alloc_vector<Vec3_t>& bearings, const Mat33_t& rot,
    const Vec3_t& trans) {
...
void pnp_solver::estimate_R_and_t(const eigen_alloc_vector<Vec3_t>& pws,
                                  const eigen_alloc_vector<Vec3_t>& pcs,
                                  Mat33_t& rot, Vec3_t& trans) {
  const unsigned int num_correspondences = pws.size();
...
Vec4_t pnp_solver::find_initial_betas(const MatRC_t<6, 10>& L_6x10,
                                      const Vec6_t& Rho, unsigned int N) {
  assert(2 <= N && N <= 4);
  if (N == 2) {
...
Vec4_t pnp_solver::find_initial_betas_2(const MatRC_t<6, 10>& L_6x10,
                                        const Vec6_t& Rho) {
  Vec4_t betas;
  MatRC_t<6, 3> L_6x3;
  ...
  Eigen::JacobiSVD<MatX_t> SVD(L_6x3,
                               Eigen::ComputeFullV | Eigen::ComputeFullU);
...
Vec4_t pnp_solver::find_initial_betas_3(const MatRC_t<6, 10>& L_6x10,
                                        const Vec6_t& Rho) {
  Vec4_t betas;
  ...
  Eigen::JacobiSVD<MatX_t> SVD(L_6x5,
                               Eigen::ComputeFullV | Eigen::ComputeFullU);
...
Vec4_t pnp_solver::find_initial_betas_4(const MatRC_t<6, 10>& L_6x10,
                                        const Vec6_t& Rho) {
  Vec4_t betas;
  MatRC_t<6, 4> L_6x4;
  ...
  Eigen::JacobiSVD<MatX_t> SVD(L_6x4,
                               Eigen::ComputeFullV | Eigen::ComputeFullU);
...
void pnp_solver::compute_A_and_b_for_gauss_newton(const MatRC_t<6, 10>& L_6x10,
                                                  const Vec6_t& Rho,
                                                  const Vec4_t& betas,
                                                  MatRC_t<6, 4>& A, Vec6_t& b) {
  for (unsigned int i = 0; i < 6; ++i) {
...
Vec4_t pnp_solver::gauss_newton(const MatRC_t<6, 10>& L_6x10, const Vec6_t& Rho,
                                const Vec4_t& initial_betas,
                                const unsigned int num_iter) {
