...
class pnp_solver {
 ...
 private:
  ...
  //! (Note: inlier flags are set to_inlier_match and the number of inliers is
  //! returned)
  unsigned int check_inliers(const Mat33_t& rot_cw, const Vec3_t& trans_cw,
  ...
  //! (Note: if the number of inliers is less than this, the solution is
  //! regarded as invalid)
 ...
 private:
  //! Reprojecton error on a virtual camera projection surface (intrinsic params
  //! are fx_, fy_, cx_ and cy_)
  static double reprojection_error(const eigen_alloc_vector<Vec3_t>& pws,
                                   const eigen_alloc_vector<Vec3_t>& bearings,
                                   const Mat33_t& rot, const Vec3_t& trans);

  //! Choose control points on the world coordinate
  static eigen_alloc_vector<Vec3_t> choose_control_points(
      const eigen_alloc_vector<Vec3_t>& pos_ws);

  //! Compute the barycentric coordinate for each world point using control
  //! points
  static eigen_alloc_vector<Vec4_t> compute_barycentric_coordinates(
  ...
  static eigen_alloc_vector<Vec3_t> compute_ccs(const Vec4_t& betas,
                                                const MatX_t& U);

  //! Compute local 3D points by utilize barycentric coordinates(alphas) and
  //! local control points(ccs)
  static eigen_alloc_vector<Vec3_t> compute_pcs(
      const eigen_alloc_vector<Vec4_t>& alphas,
      const eigen_alloc_vector<Vec3_t>& ccs, const bool bearing_z_sign);

  //! Find the coarse value of betas which are coefficients of the basis of the
  //! local control points
  static Vec4_t find_initial_betas(const MatRC_t<6, 10>& L_6x10,
                                   const Vec6_t& Rho, unsigned int N);

  //! Find the coarse value of betas in the case of N (the number of the
  //! non-null space of M) is 2
  static Vec4_t find_initial_betas_2(const MatRC_t<6, 10>& L_6x10,
                                     const Vec6_t& Rho);
  //! Find the coarse value of betas in the case of N is 3
  static Vec4_t find_initial_betas_3(const MatRC_t<6, 10>& L_6x10,
                                     const Vec6_t& Rho);
  //! Find the coarse value of betas in the case of N is 4
  static Vec4_t find_initial_betas_4(const MatRC_t<6, 10>& L_6x10,
                                     const Vec6_t& Rho);
  ...
  static Vec4_t gauss_newton(const MatRC_t<6, 10>& L_6x10, const Vec6_t& Rho,
                             const Vec4_t& betas, unsigned int num_iter = 5);

  //! Compute A matrix and b vector used for the gauss-newton algorithm
  static void compute_A_and_b_for_gauss_newton(const MatRC_t<6, 10>& L_6x10,
                                               const Vec6_t& Rho,
                                               const Vec4_t& betas,
                                               MatRC_t<6, 4>& A, Vec6_t& b);

  //! Estimate R and t by the local 3D points and the world 3D points
  static void estimate_R_and_t(const eigen_alloc_vector<Vec3_t>& pws,
