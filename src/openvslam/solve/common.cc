...
void normalize(const std::vector<cv::KeyPoint>& keypts,
  ...
  // compute centroids
  const auto pts_mean =
      std::accumulate(keypts.begin(), keypts.end(), cv::Point2f{0.0f, 0.0f},
                      [](const cv::Point2f& acc, const cv::KeyPoint& keypt) {
                        return acc + keypt.pt;
                      }) /
  ...
  for (unsigned int idx = 0; idx < num_keypts; ++idx) {
    ...
    pts_l1_dev += cv::Point2f{std::abs(normalized_pts.at(idx).x),
                              std::abs(normalized_pts.at(idx).y)};
  }
  pts_l1_dev /= static_cast<double>(num_keypts);

  // apply normalization
  std::transform(normalized_pts.begin(), normalized_pts.end(),
                 normalized_pts.begin(), [&pts_l1_dev](const cv::Point2f& pt) {
                   return cv::Point2f{pt.x / pts_l1_dev.x, pt.y / pts_l1_dev.y};
                 });
