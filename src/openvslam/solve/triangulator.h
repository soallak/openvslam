...
class triangulator {
 public:
   ...
   */
  static inline Vec3_t triangulate(const cv::Point2d& pt_1,
                                   const cv::Point2d& pt_2, const Mat34_t& P_1,
                                   const Mat34_t& P_2);
   ...
   */
  static inline Vec3_t triangulate(const Vec3_t& bearing_1,
                                   const Vec3_t& bearing_2,
  ...
  static inline Vec3_t triangulate(const Vec3_t& bearing_1,
                                   const Vec3_t& bearing_2,
                                   const Mat44_t& cam_pose_1,
                                   const Mat44_t& cam_pose_2);
};

Vec3_t triangulator::triangulate(const cv::Point2d& pt_1,
                                 const cv::Point2d& pt_2, const Mat34_t& P_1,
                                 const Mat34_t& P_2) {
  Mat44_t A;
  ...
  const Eigen::JacobiSVD<Mat44_t> svd(
...
Vec3_t triangulator::triangulate(const Vec3_t& bearing_1,
                                 const Vec3_t& bearing_2, const Mat33_t& rot_21,
                                 const Vec3_t& trans_21) {
...
Vec3_t triangulator::triangulate(const Vec3_t& bearing_1,
  ...
  Mat44_t A;
  A.row(0) =
      bearing_1(0) * cam_pose_1.row(2) - bearing_1(2) * cam_pose_1.row(0);
  A.row(1) =
      bearing_1(1) * cam_pose_1.row(2) - bearing_1(2) * cam_pose_1.row(1);
  A.row(2) =
      bearing_2(0) * cam_pose_2.row(2) - bearing_2(2) * cam_pose_2.row(0);
  A.row(3) =
      bearing_2(1) * cam_pose_2.row(2) - bearing_2(2) * cam_pose_2.row(1);
