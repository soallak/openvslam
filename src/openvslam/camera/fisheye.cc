...
fisheye::fisheye(const std::string& name, const setup_type_t& setup_type,
                 const color_order_t& color_order, const unsigned int cols,
                 const unsigned int rows, const double fps, const double fx,
                 const double fy, const double cx, const double cy,
                 const double k1, const double k2, const double k3,
                 const double k4, const double focal_x_baseline,
                 const double depth_thr)
    : base(name, setup_type, model_type_t::Fisheye, color_order, cols, rows,
           fps, focal_x_baseline, focal_x_baseline / fx, depth_thr),
      fx_(fx),
      fy_(fy),
      cx_(cx),
      cy_(cy),
...
fisheye::fisheye(const YAML::Node& yaml_node)
    : fisheye(yaml_node["name"].as<std::string>(), load_setup_type(yaml_node),
              load_color_order(yaml_node), yaml_node["cols"].as<unsigned int>(),
              yaml_node["rows"].as<unsigned int>(),
              yaml_node["fps"].as<double>(), yaml_node["fx"].as<double>(),
              yaml_node["fy"].as<double>(), yaml_node["cx"].as<double>(),
              yaml_node["cy"].as<double>(), yaml_node["k1"].as<double>(),
              yaml_node["k2"].as<double>(), yaml_node["k3"].as<double>(),
...
image_bounds fisheye::compute_image_bounds() const {
  ...
  if (k1_ == 0 && k2_ == 0 && k3_ == 0 && k4_ == 0) {
    ...
    // fix for issue #83
    // check if fov is super wide (four corners are out of view) based on
    // upper-left corner
    ...
    if (theta_d > M_PI / 2) {
      ...
      //    some points over 180 deg are projected on the opposite side (plus or
      //    minus are different)
      ...
      return image_bounds{
          (undist_min_x < min_x_thr || undist_min_x > cx_) ? min_x_thr
                                                           : undist_min_x,
          (undist_max_x > max_x_thr || undist_max_x < cx_) ? max_x_thr
                                                           : undist_max_x,
          (undist_min_y < min_y_thr || undist_min_y > cy_) ? min_y_thr
                                                           : undist_min_y,
          (undist_max_y > max_y_thr || undist_max_y < cy_) ? max_y_thr
                                                           : undist_max_y};
    } else {
...
cv::Point2f fisheye::undistort_point(const cv::Point2f& dist_pt) const {
  ...
  // undistort
  mat = mat.reshape(2);
  cv::fisheye::undistortPoints(mat, mat, cv_cam_matrix_, cv_dist_params_,
...
bool fisheye::reproject_to_image(const Mat33_t& rot_cw, const Vec3_t& trans_cw,
...
bool fisheye::reproject_to_bearing(const Mat33_t& rot_cw,
...
void fisheye::undistort_points(const std::vector<cv::Point2f>& dist_pts,
...
void fisheye::undistort_keypoints(
    ...
    std::vector<cv::KeyPoint>& undist_keypt) const {
  // cv::fisheye::undistortPoints does not accept an empty input
  if (dist_keypt.empty()) {
    undist_keypt.clear();
    return;
