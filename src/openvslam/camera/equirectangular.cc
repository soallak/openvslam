...
equirectangular::equirectangular(const std::string& name,
    ...
    : base(name, setup_type_t::Monocular, model_type_t::Equirectangular,
           color_order, cols, rows, fps, 0.0, 0.0, 0.0) {
...
equirectangular::equirectangular(const YAML::Node& yaml_node)
    : equirectangular(
          yaml_node["name"].as<std::string>(), load_color_order(yaml_node),
...
Vec3_t equirectangular::convert_point_to_bearing(
  ...
  return Vec3_t{std::cos(lat) * std::sin(lon), -std::sin(lat),
                std::cos(lat) * std::cos(lon)};
}

cv::Point2f equirectangular::convert_bearing_to_point(
  ...
  return cv::Point2f(cols_ * (0.5 + lon / (2.0 * M_PI)),
                     rows_ * (0.5 - lat / M_PI));
}

bool equirectangular::reproject_to_image(const Mat33_t& rot_cw,
...
bool equirectangular::reproject_to_bearing(const Mat33_t& rot_cw,
                                           const Vec3_t& trans_cw,
                                           const Vec3_t& pos_w,
                                           Vec3_t& reproj) const {
  // convert to camera-coordinates
...
void equirectangular::undistort_points(
