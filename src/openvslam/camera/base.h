...
enum class setup_type_t { Monocular = 0, Stereo = 1, RGBD = 2 };

const std::array<std::string, 3> setup_type_to_string = {
...
const std::array<std::string, 4> model_type_to_string = {
    {"Perspective", "Fisheye", "Equirectangular", "RadialDivision"}};

enum class color_order_t { Gray = 0, RGB = 1, BGR = 2 };

const std::array<std::string, 3> color_order_to_string = {
    {"Gray", "RGB", "BGR"}};
...
class base {
 public:
  //! Constructor
  base(const std::string& name, const setup_type_t setup_type,
       ...
       const double depth_thr, const unsigned int num_grid_cols = 64,
       const unsigned int num_grid_rows = 48);
  ...
  //! depth threshold in metric scale (Ignore depths farther than depth_thr_
  //! times the baseline. if a stereo-triangulated point is farther than this
  ...
  virtual Vec3_t convert_point_to_bearing(
      const cv::Point2f& undist_pt) const = 0;

  //! Convert bearing vector to undistorted point
  virtual cv::Point2f convert_bearing_to_point(const Vec3_t& bearing) const = 0;

  //! Reproject the specified 3D point to image using camera pose and projection
  //! model (reprojected to inside of image -> true, to outside of image ->
  //! false)
  virtual bool reproject_to_image(const Mat33_t& rot_cw, const Vec3_t& trans_cw,
                                  const Vec3_t& pos_w, Vec2_t& reproj,
                                  float& x_right) const = 0;

  //! Reproject the specified 3D point to bearing vector using camera pose (Not
  //! depends on any projection models) (reprojected to inside of image -> true,
  //! to outside of image -> false)
  virtual bool reproject_to_bearing(const Mat33_t& rot_cw,
                                    const Vec3_t& trans_cw, const Vec3_t& pos_w,
  ...
  virtual void undistort_points(const std::vector<cv::Point2f>& dist_pts,
  ...
  virtual void convert_points_to_bearings(
  ...
  virtual void convert_bearings_to_points(
