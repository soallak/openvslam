...
perspective::perspective(const std::string& name,
                         const setup_type_t& setup_type,
                         const color_order_t& color_order,
                         const unsigned int cols, const unsigned int rows,
                         const double fps, const double fx, const double fy,
                         const double cx, const double cy, const double k1,
                         const double k2, const double p1, const double p2,
                         const double k3, const double focal_x_baseline,
                         const double depth_thr)
    : base(name, setup_type, model_type_t::Perspective, color_order, cols, rows,
...
perspective::perspective(const YAML::Node& yaml_node)
    : perspective(yaml_node["name"].as<std::string>(),
                  load_setup_type(yaml_node), load_color_order(yaml_node),
                  ...
                  yaml_node["k2"].as<double>(), yaml_node["p1"].as<double>(),
                  yaml_node["p2"].as<double>(), yaml_node["k3"].as<double>(),
...
Vec3_t perspective::convert_point_to_bearing(
...
bool perspective::reproject_to_bearing(const Mat33_t& rot_cw,
                                       const Vec3_t& trans_cw,
                                       const Vec3_t& pos_w,
                                       Vec3_t& reproj) const {
  // convert to camera-coordinates
...
void perspective::undistort_points(const std::vector<cv::Point2f>& dist_pts,
