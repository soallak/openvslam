...
base::base(const std::string& name, const setup_type_t setup_type,
           const model_type_t model_type, const color_order_t color_order,
           const unsigned int cols, const unsigned int rows, const double fps,
           const double focal_x_baseline, const double true_baseline,
           const double depth_thr, const unsigned int num_grid_cols,
           const unsigned int num_grid_rows)
    : name_(name),
...
setup_type_t base::load_setup_type(const std::string& setup_type_str) {
  const auto itr = std::find(setup_type_to_string.begin(),
                             setup_type_to_string.end(), setup_type_str);
  if (itr == setup_type_to_string.end()) {
    throw std::runtime_error("Invalid setup type: " + setup_type_str);
  }
  return static_cast<setup_type_t>(
      std::distance(setup_type_to_string.begin(), itr));
}
...
model_type_t base::load_model_type(const std::string& model_type_str) {
  const auto itr = std::find(model_type_to_string.begin(),
                             model_type_to_string.end(), model_type_str);
  if (itr == model_type_to_string.end()) {
    throw std::runtime_error("Invalid camera model: " + model_type_str);
  }
  return static_cast<model_type_t>(
      std::distance(model_type_to_string.begin(), itr));
}
...
color_order_t base::load_color_order(const std::string& color_order_str) {
  const auto itr = std::find(color_order_to_string.begin(),
                             color_order_to_string.end(), color_order_str);
  if (itr == color_order_to_string.end()) {
    throw std::runtime_error("Invalid color order: " + color_order_str);
  }
  return static_cast<color_order_t>(
      std::distance(color_order_to_string.begin(), itr));
}
...
void base::undistort_points(const std::vector<cv::Point2f>& dist_pts,
...
void base::convert_points_to_bearings(
    const std::vector<cv::Point2f>& undist_pts,
    eigen_alloc_vector<Vec3_t>& bearings) const {
  std::transform(undist_pts.begin(), undist_pts.end(),
                 std::back_inserter(bearings),
                 [this](const cv::Point2f& undist_pt) {
                   return convert_point_to_bearing(undist_pt);
                 });
}

void base::convert_keypoints_to_bearings(
    const std::vector<cv::KeyPoint>& undist_keypts,
    eigen_alloc_vector<Vec3_t>& bearings) const {
  std::transform(undist_keypts.begin(), undist_keypts.end(),
                 std::back_inserter(bearings),
                 [this](const cv::KeyPoint& undist_keypt) {
                   return convert_point_to_bearing(undist_keypt.pt);
                 });
}

void base::convert_bearings_to_points(
    const eigen_alloc_vector<Vec3_t>& bearings,
    std::vector<cv::Point2f>& undist_pts) const {
  std::transform(bearings.begin(), bearings.end(),
                 std::back_inserter(undist_pts), [this](const Vec3_t& bearing) {
                   return convert_bearing_to_point(bearing);
                 });
}
