...
class mono_perspective_pose_opt_edge final
...
inline Vec2_t mono_perspective_pose_opt_edge::cam_project(
    const Vec3_t& pos_c) const {
  return {fx_ * pos_c(0) / pos_c(2) + cx_, fy_ * pos_c(1) / pos_c(2) + cy_};
}

class stereo_perspective_pose_opt_edge
    : public g2o::BaseUnaryEdge<3, Vec3_t, shot_vertex> {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  Vec3_t pos_w_;
  g2o::number_t fx_, fy_, cx_, cy_, focal_x_baseline_;
};
...
inline void stereo_perspective_pose_opt_edge::linearizeOplus() {
  ...
  _jacobianOplusXi(2, 0) =
      _jacobianOplusXi(0, 0) - focal_x_baseline_ * y / z_sq;
  _jacobianOplusXi(2, 1) =
      _jacobianOplusXi(0, 1) + focal_x_baseline_ * x / z_sq;
...
inline Vec3_t stereo_perspective_pose_opt_edge::cam_project(
    const Vec3_t& pos_c) const {
