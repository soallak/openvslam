...
class shot_vertex_container {
 public:
  //! Constructor
  explicit shot_vertex_container(const std::shared_ptr<unsigned int> offset,
                                 const unsigned int num_reserve = 50);
  ...
  shot_vertex* create_vertex(const std::shared_ptr<data::keyframe>& keyfrm,
                             const bool is_constant);

  //! Create and return the g2o vertex created from shot ID and camera pose
  shot_vertex* create_vertex(const unsigned int id, const Mat44_t& cam_pose_cw,
                             const bool is_constant);
  ...
  using const_iterator =
      std::unordered_map<unsigned int, shot_vertex*>::const_iterator;
  iterator begin();
...
inline shot_vertex_container::shot_vertex_container(
...
inline shot_vertex* shot_vertex_container::create_vertex(
    data::frame* frm, const bool is_constant) {
  return create_vertex(frm->id_, frm->cam_pose_cw_, is_constant);
}

inline shot_vertex* shot_vertex_container::create_vertex(
    const std::shared_ptr<data::keyframe>& keyfrm, const bool is_constant) {
  return create_vertex(keyfrm->id_, keyfrm->get_cam_pose(), is_constant);
}

inline shot_vertex* shot_vertex_container::create_vertex(
    const unsigned int id, const Mat44_t& cam_pose_cw, const bool is_constant) {
  // vertexを作成
...
inline shot_vertex* shot_vertex_container::get_vertex(
    const std::shared_ptr<data::keyframe>& keyfrm) const {
...
inline unsigned int shot_vertex_container::get_vertex_id(
    data::frame* frm) const {
...
inline bool shot_vertex_container::contain(
    const std::shared_ptr<data::keyframe>& keyfrm) const {
...
inline shot_vertex_container::const_iterator shot_vertex_container::begin()
    const {
  return vtx_container_.begin();
}
...
inline shot_vertex_container::const_iterator shot_vertex_container::end()
    const {
  return vtx_container_.end();
}
