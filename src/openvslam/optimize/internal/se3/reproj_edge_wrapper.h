...
class reproj_edge_wrapper {
 public:
  ...
  reproj_edge_wrapper(const std::shared_ptr<T>& shot, shot_vertex* shot_vtx,
                      const std::shared_ptr<data::landmark>& lm,
                      landmark_vertex* lm_vtx, const unsigned int idx,
                      const float obs_x, const float obs_y,
                      const float obs_x_right, const float inv_sigma_sq,
...
template <typename T>
inline reproj_edge_wrapper<T>::reproj_edge_wrapper(
    const std::shared_ptr<T>& shot, shot_vertex* shot_vtx,
    const std::shared_ptr<data::landmark>& lm, landmark_vertex* lm_vtx,
    const unsigned int idx, const float obs_x, const float obs_y,
    const float obs_x_right, const float inv_sigma_sq, const float sqrt_chi_sq,
