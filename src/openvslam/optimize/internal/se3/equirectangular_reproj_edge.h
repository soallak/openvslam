...
inline void equirectangular_reproj_edge::linearizeOplus() {
  ...
  d_pcx_d_x << d_pc_d_rx(0), d_pc_d_ry(0), d_pc_d_rz(0), d_pc_d_tx(0),
      d_pc_d_ty(0), d_pc_d_tz(0), d_pc_d_pwx(0), d_pc_d_pwy(0), d_pc_d_pwz(0);
  VecR_t<9> d_pcy_d_x;
  d_pcy_d_x << d_pc_d_rx(1), d_pc_d_ry(1), d_pc_d_rz(1), d_pc_d_tx(1),
      d_pc_d_ty(1), d_pc_d_tz(1), d_pc_d_pwx(1), d_pc_d_pwy(1), d_pc_d_pwz(1);
  VecR_t<9> d_pcz_d_x;
  d_pcz_d_x << d_pc_d_rx(2), d_pc_d_ry(2), d_pc_d_rz(2), d_pc_d_tx(2),
      d_pc_d_ty(2), d_pc_d_tz(2), d_pc_d_pwx(2), d_pc_d_pwy(2), d_pc_d_pwz(2);

  // 導関数ベクトル d_L_d_x を作成
  const VecR_t<9> d_L_d_x =
      (1.0 / L) * (pcx * d_pcx_d_x + pcy * d_pcy_d_x + pcz * d_pcz_d_x);

  // ヤコビ行列を作成
  MatRC_t<2, 9> jacobian = MatRC_t<2, 9>::Zero();
  jacobian.block<1, 9>(0, 0) = -(cols_ / (2 * M_PI)) *
                               (1.0 / (pcx * pcx + pcz * pcz)) *
                               (pcz * d_pcx_d_x - pcx * d_pcz_d_x);
  jacobian.block<1, 9>(1, 0) = -(rows_ / M_PI) *
...
inline Vec2_t equirectangular_reproj_edge::cam_project(
