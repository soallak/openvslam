...
class base_forward_reproj_edge
...
inline Vec2_t perspective_forward_reproj_edge::cam_project(
    const Vec3_t& pos_c) const {
  return {fx_ * pos_c(0) / pos_c(2) + cx_, fy_ * pos_c(1) / pos_c(2) + cy_};
}

class equirectangular_forward_reproj_edge final
    : public base_forward_reproj_edge {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
...
inline equirectangular_forward_reproj_edge::
    equirectangular_forward_reproj_edge()
    : base_forward_reproj_edge() {}

inline Vec2_t equirectangular_forward_reproj_edge::cam_project(
    const Vec3_t& pos_c) const {
