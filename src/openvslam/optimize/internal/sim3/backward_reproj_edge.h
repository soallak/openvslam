...
class base_backward_reproj_edge
    : public g2o::BaseUnaryEdge<2, Vec2_t, transform_vertex> {
 public:
...
class perspective_backward_reproj_edge final
    : public base_backward_reproj_edge {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
...
inline Vec2_t perspective_backward_reproj_edge::cam_project(
    const Vec3_t& pos_c) const {
  return {fx_ * pos_c(0) / pos_c(2) + cx_, fy_ * pos_c(1) / pos_c(2) + cy_};
}

class equirectangular_backward_reproj_edge final
    : public base_backward_reproj_edge {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
...
inline equirectangular_backward_reproj_edge::
    equirectangular_backward_reproj_edge()
    : base_backward_reproj_edge() {}

inline Vec2_t equirectangular_backward_reproj_edge::cam_project(
    const Vec3_t& pos_c) const {
