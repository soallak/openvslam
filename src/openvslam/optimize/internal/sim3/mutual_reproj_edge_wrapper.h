...
class mutual_reproj_edge_wapper {
 public:
  ...
  mutual_reproj_edge_wapper(const std::shared_ptr<T>& shot1, unsigned int idx1,
                            const std::shared_ptr<data::landmark>& lm1,
                            const std::shared_ptr<T>& shot2, unsigned int idx2,
                            const std::shared_ptr<data::landmark>& lm2,
...
template <typename T>
inline mutual_reproj_edge_wapper<T>::mutual_reproj_edge_wapper(
    const std::shared_ptr<T>& shot1, unsigned int idx1,
    const std::shared_ptr<data::landmark>& lm1, const std::shared_ptr<T>& shot2,
    unsigned int idx2, const std::shared_ptr<data::landmark>& lm2,
  ...
  // 1. forward edgeを作成
  {
    ...
    switch (camera1->model_type_) {
      case camera::model_type_t::Perspective: {
        ...
        const float inv_sigma_sq_1 =
            shot1->orb_params_->inv_level_sigma_sq_.at(undist_keypt_1.octave);
      ...
      case camera::model_type_t::Equirectangular: {
        ...
        // 3次元点はkeyfrm_2で観測しているもの，カメラモデルと特徴点はkeyfrm_1のもの
        auto edge_12 =
            new internal::sim3::equirectangular_forward_reproj_edge();
        // 特徴点情報と再投影誤差分散をセット
        const auto& undist_keypt_1 = shot1->frm_obs_.undist_keypts_.at(idx1);
        const Vec2_t obs_1{undist_keypt_1.pt.x, undist_keypt_1.pt.y};
        const float inv_sigma_sq_1 =
  ...
  // 2. backward edgeを作成
  {
    ...
    switch (camera2->model_type_) {
      case camera::model_type_t::Perspective: {
        ...
        const float inv_sigma_sq_2 =
            shot2->orb_params_->inv_level_sigma_sq_.at(undist_keypt_2.octave);
      ...
      case camera::model_type_t::Equirectangular: {
        ...
        // 3次元点はkeyfrm_1で観測しているもの，カメラモデルと特徴点はkeyfrm_2のもの
        auto edge_21 =
            new internal::sim3::equirectangular_backward_reproj_edge();
        // 特徴点情報と再投影誤差分散をセット
        const auto& undist_keypt_2 = shot2->frm_obs_.undist_keypts_.at(idx2);
        const Vec2_t obs_2{undist_keypt_2.pt.x, undist_keypt_2.pt.y};
        const float inv_sigma_sq_2 =
