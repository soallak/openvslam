...
class landmark_vertex_container {
 public:
  //! Constructor
  explicit landmark_vertex_container(const std::shared_ptr<unsigned int> offset,
                                     const unsigned int num_reserve = 200);

  //! Destructor
  virtual ~landmark_vertex_container() = default;

  //! Create and return the g2o vertex created from the specified landmark
  landmark_vertex* create_vertex(const std::shared_ptr<data::landmark>& lm,
                                 const bool is_constant);

  //! Create and return the g2o vertex created from the specified landmark
  landmark_vertex* create_vertex(const unsigned int id, const Vec3_t& pos_w,
                                 const bool is_constant);
  ...
  using const_iterator =
      std::unordered_map<unsigned int, landmark_vertex*>::const_iterator;
  iterator begin();
...
inline landmark_vertex_container::landmark_vertex_container(
    const std::shared_ptr<unsigned int> offset, const unsigned int num_reserve)
    : offset_(offset) {
...
inline landmark_vertex* landmark_vertex_container::create_vertex(
    const std::shared_ptr<data::landmark>& lm, const bool is_constant) {
  return create_vertex(lm->id_, lm->get_pos_in_world(), is_constant);
}

inline landmark_vertex* landmark_vertex_container::create_vertex(
    const unsigned int id, const Vec3_t& pos_w, const bool is_constant) {
  // vertexを作成
...
inline landmark_vertex* landmark_vertex_container::get_vertex(
    const std::shared_ptr<data::landmark>& lm) const {
...
inline unsigned int landmark_vertex_container::get_vertex_id(
    const std::shared_ptr<data::landmark>& lm) const {
...
inline unsigned int landmark_vertex_container::get_id(
    landmark_vertex* vtx) const {
  return get_id(vtx->id());
}

inline unsigned int landmark_vertex_container::get_id(
    const unsigned int vtx_id) const {
  return id_container_.at(vtx_id);
}

inline bool landmark_vertex_container::contain(
    const std::shared_ptr<data::landmark>& lm) const {
...
inline landmark_vertex_container::const_iterator
landmark_vertex_container::begin() const {
  return vtx_container_.begin();
}
...
inline landmark_vertex_container::const_iterator
landmark_vertex_container::end() const {
  return vtx_container_.end();
}
