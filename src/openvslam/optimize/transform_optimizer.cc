...
transform_optimizer::transform_optimizer(const bool fix_scale,
...
unsigned int transform_optimizer::optimize(
  ...
  // - backward: an edge reprojecting 3D point observed keyframe 2 to keyframe 1
  // (using the camera model of keyframe 1)
  // - forward: an edge reprojecting 3D point observed keyframe 1 to keyframe 2
  // (using the camera model of keyframe 2)
  using reproj_edge_wrapper =
      internal::sim3::mutual_reproj_edge_wapper<data::keyframe>;
  ...
  for (unsigned int idx1 = 0; idx1 < num_matches; ++idx1) {
    ...
    reproj_edge_wrapper mutual_edge(keyfrm_1, idx1, lm_1, keyfrm_2, idx2, lm_2,
                                    Sim3_12_vtx, sqrt_chi_sq);
