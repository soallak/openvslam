...
class system {
 public:
  ...
  void save_keyframe_trajectory(const std::string& path,
                                const std::string& format) const;
  ...
  std::shared_ptr<Mat44_t> feed_frame(const data::frame& frm,
                                      const cv::Mat& img);

  //! Feed a monocular frame to SLAM system
  //! (NOTE: distorted images are acceptable if calibrated)
  data::frame create_monocular_frame(const cv::Mat& img, const double timestamp,
  ...
  data::frame create_stereo_frame(const cv::Mat& left_img,
                                  const cv::Mat& right_img,
                                  const double timestamp,
                                  const cv::Mat& mask = cv::Mat{});
  std::shared_ptr<Mat44_t> feed_stereo_frame(const cv::Mat& left_img,
                                             const cv::Mat& right_img,
                                             const double timestamp,
                                             const cv::Mat& mask = cv::Mat{});
  ...
  data::frame create_stereo_disparity_frame(const cv::Mat& left_img,
                                            const cv::Mat& disparity_img,
                                            const double timestamp,
                                            const cv::Mat& mask = cv::Mat{});
  std::shared_ptr<Mat44_t> feed_stereo_disparity_frame(
  ...
  data::frame create_RGBD_frame(const cv::Mat& rgb_img, const cv::Mat& depthmap,
                                const double timestamp, const cv::Mat& mask);
  std::shared_ptr<Mat44_t> feed_RGBD_frame(const cv::Mat& rgb_img,
  ...
  bool relocalize_by_pose_2d(const Mat44_t& cam_pose_wc,
