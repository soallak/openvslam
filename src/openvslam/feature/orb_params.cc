...
orb_params::orb_params(const std::string& name, const float scale_factor,
...
std::vector<float> orb_params::calc_inv_scale_factors(
  ...
  for (unsigned int level = 1; level < num_scale_levels; ++level) {
    inv_scale_factors.at(level) =
        (1.0f / scale_factor) * inv_scale_factors.at(level - 1);
  }
