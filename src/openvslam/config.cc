...
config::config(const YAML::Node& yaml_node, const std::string& config_file_path)
  ...
  const auto camera_model_type =
      camera::base::load_model_type(yaml_node_["Camera"]);
  ...
  try {
    orb_params_ =
        new feature::orb_params(util::yaml_optional_ref(yaml_node_, "Feature"));
  } catch (const std::exception& e) {
    spdlog::debug("failed in loading ORB feature extraction model: {}",
                  e.what());
    if (orb_params_) {
      delete orb_params_;
      orb_params_ = nullptr;
