...
template <typename T>
std::vector<T> create_random_array(const size_t size, const T rand_min,
                                   const T rand_max,
                                   std::mt19937& random_engine) {
  ...
  // Create a random sequence slightly larger than the 'size' (because
  // duplication possibly exists)
  ...
  while (v.size() != size) {
    ...
    // Sort to remove duplicates so that the last iterator of the deduplicated
    // sequence goes into 'unique_end'
    std::sort(v.begin(), v.end());
    auto unique_end = std::unique(v.begin(), v.end());

    // If the vector size is too large, change it to an iterator up to the
    // 'size'
...
template std::vector<unsigned int> create_random_array(size_t, unsigned int,
                                                       unsigned int,
                                                       std::mt19937&);
