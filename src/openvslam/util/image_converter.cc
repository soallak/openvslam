...
void convert_to_grayscale(cv::Mat& img,
                          const camera::color_order_t in_color_order) {
  if (img.channels() == 3) {
...
void equalize_histogram(cv::Mat& img) {
  ...
  if (img.type() == CV_16UC1) {
    std::vector<unsigned short> vec(img.begin<unsigned short>(),
                                    img.end<unsigned short>());
    ...
    img.convertTo(img, CV_8UC1, 255.0 / (h - l),
                  -255.0 * l / (h - l));  // 255*(img-l)/(h-l)
  } else if (img.type() == CV_8UC1) {
    std::vector<unsigned char> vec(img.begin<unsigned char>(),
