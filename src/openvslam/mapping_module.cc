...
mapping_module::mapping_module(const YAML::Node& yaml_node,
                               data::map_database* map_db,
                               data::bow_database* bow_db,
                               data::bow_vocabulary* bow_vocab)
    : local_map_cleaner_(new module::local_map_cleaner(
          map_db, bow_db,
          yaml_node["redundant_obs_ratio_thr"].as<double>(0.9))),
      map_db_(map_db),
      bow_db_(bow_db),
  ...
  if (yaml_node["baseline_dist_thr"]) {
    if (yaml_node["baseline_dist_thr_ratio"]) {
      throw std::runtime_error(
          "Do not set both baseline_dist_thr_ratio and baseline_dist_thr.");
    }
    ...
    baseline_dist_thr_ratio_ =
        yaml_node["baseline_dist_thr_ratio"].as<double>(0.02);
...
void mapping_module::set_global_optimization_module(
...
void mapping_module::run() {
  ...
  while (true) {
    ...
    if (pause_is_requested_and_not_prevented()) {
      set_is_idle(false);
      // if any keyframe is queued, all of them must be processed before the
      // pause
...
void mapping_module::queue_keyframe(
...
bool mapping_module::is_idle() const { return is_idle_; }

void mapping_module::set_is_idle(const bool is_idle) { is_idle_ = is_idle; }
...
void mapping_module::mapping_with_new_keyframe() {
  ...
  local_map_cleaner_->remove_redundant_landmarks(
      cur_keyfrm_->id_, cur_keyfrm_->depth_is_avaliable());

  // triangulate new landmarks between the current frame and each of the
  // covisibilities
  ...
  // detect and resolve the duplication of the landmarks observed in the current
  // frame
  update_new_keyframe();
...
void mapping_module::store_new_keyframe() {
  ...
  for (unsigned int idx = 0; idx < cur_lms.size(); ++idx) {
    ...
    if (lm->is_observed_in_keyframe(cur_keyfrm_)) {
      // if `lm` is correctly observed, make it be checked by the local map
      // cleaner
...
void mapping_module::create_new_landmarks() {
  ...
  // in order to triangulate landmarks between `cur_keyfrm_` and each of the
  // covisibilities
  constexpr unsigned int num_covisibilities = 10;
  const unsigned int heuristic_ratio =
      cur_keyfrm_->depth_is_avaliable() ? 1 : 2;
  const auto cur_covisibilities =
      cur_keyfrm_->graph_node_->get_top_n_covisibilities(num_covisibilities *
                                                         heuristic_ratio);
  ...
  for (unsigned int i = 0; i < cur_covisibilities.size(); ++i) {
    ...
    // if the scene scale is much smaller than the baseline, abort the
    // triangulation
    ...
    // solve::essential_solver::create_E_21(ngh_keyfrm, cur_keyfrm_);
    const Mat33_t E_ngh_to_cur = solve::essential_solver::create_E_21(
        ngh_keyfrm->get_rotation(), ngh_keyfrm->get_translation(),
    ...
    robust_matcher.match_for_triangulation(cur_keyfrm_, ngh_keyfrm,
                                           E_ngh_to_cur, matches);
...
void mapping_module::update_new_keyframe() {
  ...
  const unsigned int num_covisibilities =
      cur_keyfrm_->depth_is_avaliable() ? 10 : 20;
  const auto fuse_tgt_keyfrms =
      get_second_order_covisibilities(num_covisibilities, 5);

  // resolve the duplication of landmarks between the current keyframe and the
  // targets
...
mapping_module::get_second_order_covisibilities(
  ...
  for (const auto& first_order_covis : cur_covisibilities) {
    ...
    const auto ngh_covisibilities =
        first_order_covis->graph_node_->get_top_n_covisibilities(
            second_order_thr);
...
void mapping_module::fuse_landmark_duplication(
  ...
  {
    // reproject the landmarks observed in the current keyframe to each of the
    // targets, and acquire
    // - additional matches
  ...
  {
    // reproject the landmarks observed in each of the targets to each of the
    // current frame, and acquire
    // - additional matches
    // - duplication of matches
    // then, add matches and solve duplication
    std::unordered_set<std::shared_ptr<data::landmark>>
        candidate_landmarks_to_fuse;
    candidate_landmarks_to_fuse.reserve(fuse_tgt_keyfrms.size() *
                                        cur_keyfrm_->frm_obs_.num_keypts_);
