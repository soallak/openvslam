...
viewer::viewer(
    const YAML::Node& yaml_node, openvslam::system* system,
  ...
  s_cam_ = std::unique_ptr<pangolin::OpenGlRenderState>(
      new pangolin::OpenGlRenderState(
          pangolin::ProjectionMatrix(
              map_viewer_width_, map_viewer_height_, viewpoint_f_, viewpoint_f_,
              map_viewer_width_ / 2, map_viewer_height_ / 2, 0.1, 1e6),
          pangolin::ModelViewLookAt(viewpoint_x_, viewpoint_y_, viewpoint_z_, 0,
                                    0, 0, 0.0, -1.0, 0.0)));
  ...
  pangolin::View& d_cam =
      pangolin::CreateDisplay()
          .SetBounds(0.0, 1.0, pangolin::Attach::Pix(175), 1.0,
                     -map_viewer_width_ / map_viewer_height_)
...
void viewer::create_menu_panel() {
  ...
  menu_grid_ = std::unique_ptr<pangolin::Var<bool>>(
      new pangolin::Var<bool>("menu.Show Grid", false, true));
  ...
  menu_loop_detection_mode_ =
      std::unique_ptr<pangolin::Var<bool>>(new pangolin::Var<bool>(
          "menu.Loop Detection", loop_detection_mode_, true));
...
void viewer::draw_horizontal_grid() {
  ...
  for (int x = -10; x <= 10; x += 1) {
    draw_line(x * 10.0f * interval_ratio, grid_min, 0,
              x * 10.0f * interval_ratio, grid_max, 0);
  }
  for (int y = -10; y <= 10; y += 1) {
    draw_line(grid_min, y * 10.0f * interval_ratio, 0, grid_max,
              y * 10.0f * interval_ratio, 0);
  }
...
void viewer::draw_current_cam_pose(
    const pangolin::OpenGlMatrix& gl_cam_pose_wc) {
...
void viewer::draw_keyframes() {
  ...
  if (*menu_show_graph_) {
    ...
    const auto draw_edge = [](const openvslam::Vec3_t& cam_center_1,
                              const openvslam::Vec3_t& cam_center_2) {
    ...
    for (const auto keyfrm : keyfrms) {
      ...
      // covisibility graph
      const auto covisibilities =
          keyfrm->graph_node_->get_covisibilities_over_weight(100);
      ...
      if (spanning_parent) {
        const openvslam::Vec3_t cam_center_2 =
            spanning_parent->get_cam_center();
        draw_edge(cam_center_1, cam_center_2);
      }
...
void viewer::draw_camera(const pangolin::OpenGlMatrix& gl_cam_pose_wc,
...
void viewer::draw_camera(const openvslam::Mat44_t& cam_pose_wc,
                         const float width) const {
  glPushMatrix();
